...
############################################################ БЛОК  ЗАВЕРШЕНИЯ РАБОТЫ ############################################################
    ...
    def start_all(self):
        ...
        Метод является единой точкой повторного запуска DeviceManager
        после ручной остановки системы.

        ВАЖНО:
            метод выполняется в собственном потоке DeviceManager.
        """
        ...
        # ============================================================
        #
        # Если polling уже работает, второй параллельный цикл
        # запускать нельзя.
        # ============================================================
        ...
        # ============================================================
        # 2. ПЕРЕВОДИМ DEVICEMANAGER В РАБОЧЕЕ СОСТОЯНИЕ
        # ============================================================

        self.running = True

        # ============================================================
        # 3. ОСТАНАВЛИВАЕМ ВОЗМОЖНЫЕ ОСТАТОЧНЫЕ ТАЙМЕРЫ
        # ============================================================
        #
        # После предыдущего Stop активных таймеров быть не должно.
        # Тем не менее перед новым запуском явно приводим состояние
        # к известному исходному состоянию.
        # ============================================================
        ...
        # ============================================================
        # 4. ЗАКРЫВАЕМ ОСТАТОЧНЫЙ COM-ПОРТ
        # ============================================================
        ...
        # ============================================================
        # 5. ОЧИЩАЕМ СОСТОЯНИЕ ПРЕДЫДУЩЕГО ОБМЕНА
        # ============================================================
        ...
        # ============================================================
        # 6. ЗАПУСКАЕМ НОВЫЙ ЦИКЛ
        # ============================================================

        self.dispatch_poll_step()
...
################################################ БЛОК МЕТОДОВ ДАННЫХ ПРИБОРОВ ######################################################
    ...
    def _parse_spectrum_data(self, data: bytearray) -> dict:
        """
        Разбирает ответ GET_SIMPLE_SPECTRE.

        Текущая используемая программой структура:

            1023 спектральных канала;
            время набора полученного участка;
            ПАЕД;
            точность ПАЕД;
            служебный байт.

        ВАЖНО:
            вопрос 1023/1024 физических каналов должен быть
            отдельно подтверждён по протоколу и на реальном приборе.

            До такого подтверждения существующую структуру
            1023 каналов не изменяем.
        """
        ...
        if not isinstance(data, (bytes, bytearray)):
            ...
            raise TypeError(
                "Некоректний тип даних GetSpectre."
            )
        ...
        # ============================================================
        #
        # Последний используемый текущим форматом байт:
        #
        #     data[2061]
        ...
        if len(data) < 2062:
            ...
            raise ValueError(
                (
                    "Відповідь GetSpectre занадто коротка: "
                    f"{len(data)} байт."
                )
            )

        # ============================================================
        # 3. 1023 СПЕКТРАЛЬНЫХ КАНАЛА
        # ============================================================
        ...
        # ============================================================
        # 4. ВРЕМЯ НАБОРА ТЕКУЩЕГО УЧАСТКА
        # ============================================================

        acquisition_time = struct.unpack(
            "<H",
            data[2054:2056]
        )[0]

        # Для передачи в GUI сохраняется существующий формат:
        #
        #     1023 канала + время = 1024 элемента.

        channels.append(acquisition_time)

        # ============================================================
        # 5. ПАЕД
        # ============================================================

        paed_raw = struct.unpack(
            "<I",
            data[2056:2060]
        )[0]
        ...
        if test_byte & 0x80:
            paed_value = paed_raw * 0.1
        else:
            paed_value = paed_raw * 0.01

        # ============================================================
        # 6. ВАЛИДНОСТЬ ПАЕД
        # ============================================================

        result_valid = not bool(
            test_byte & 0b00000100
        )
...
################################################ БЛОК ОБНОВЛЕНИЯ ДАННЫХ ЦИСТЕРН ######################################################
    ...
    def set_cistern_states(self, states: dict):
        """
        Обновляет состояние заполненности ZB внутри DeviceInfo.
        ...
        Значения:
            ...
            True  - PLC подтвердил, что цистерна заполнена;
            False - PLC подтвердил, что цистерна пуста;
            None  - актуальное состояние PLC неизвестно.

        ВАЖНО:

        None НЕ означает empty.

        Последнее физическое значение fullness сохраняется,
        но оно перестаёт использоваться для запуска/продолжения
        спектрального цикла.
        """
        ...
        for dev in self.devices:
            ...
            try:
                ...
                new_state = states[posit]

                # ====================================================
                # 3. PLC НЕ ПОДТВЕРЖДАЕТ FULLNESS
                # ====================================================
                #
                # None означает "неизвестно", а НЕ "empty".
                #
                # Последнее физическое значение dev.get_full()
                # специально НЕ изменяем.
                # ====================================================
                ...
                if new_state is None:
                    ...
                    dev.fullness_valid = False

                    # ------------------------------------------------
                    # При потере достоверного состояния цистерны
                    # продолжать накопление спектра нельзя.
                    # ------------------------------------------------

                    if dev.spectrum_active:
                    ...
                    if was_valid:
                        ...
                        self.device_info.emit(
                            (
                                f"ZB{posit}: актуальний стан "
                                "цистерни від PLC втрачено. "
                                "Спектральний цикл зупинено."
                            )
                        )
                ...
                # ====================================================
                # 4. ПРОВЕРЯЕМ ТИП ДОСТОВЕРНОГО СОСТОЯНИЯ
                # ====================================================
                ...
                if type(new_state) is not bool:
                    ...
                    self.device_error.emit(
                        ...
                        (
                            f"Некоректний стан fullness "
                            f"для ZB{posit}: {new_state}"
                        )
                    )
                ...
                # ====================================================
                # 5. PLC СНОВА ПОДТВЕРДИЛ FULLNESS
                # ====================================================
                ...
                dev.fullness_valid = True

                old_full = dev.get_full()
                new_full = new_state

                # ----------------------------------------------------
                # Если значение не изменилось, физический переход
                # отсутствует.
                # ----------------------------------------------------
                ...
                # Старый спектр относится к предыдущему
                # содержимому цистерны.
                dev.reset_spectrum()
...
################################################ БЛОК ЦИКЛИЧЕСКОГО ОПРОСА ПРИБОРОВ ######################################################  
    ...
    def make_request(self, request_type: str = None) -> bytearray:
        """
        Формирует очередной запрос к прибору.

        Для ZB спектральный режим разрешается только тогда, когда
        состояние fullness недавно подтверждено PLC.

        Если fullness_valid == False:

            - физическое старое значение full/empty не изменяем;
            - StartSpectre не разрешаем;
            - GetSpectre не продолжаем;
            - выполняем только RadDose.
        """
        ...
        # ============================================================
            ...
            # --------------------------------------------------------
                ...
                # ====================================================
                    ...
                    # =================================================
                    # СОСТОЯНИЕ ЦИСТЕРНЫ НЕ ПОДТВЕРЖДЕНО PLC
                    # =================================================
                    #
                    # Это принципиально отличается от empty.
                    #
                    # Мы не меняем сохранённое значение fullness,
                    # но не разрешаем использовать его для управления
                    # спектральным циклом.
                    # =================================================
                    ...
                    # =================================================
                    # PLC ПОДТВЕРЖДАЕТ, ЧТО ЦИСТЕРНА ПУСТАЯ
                    # =================================================

                    elif not device.get_full():
                    ...
                    # =================================================
                    # PLC ПОДТВЕРЖДАЕТ, ЧТО ЦИСТЕРНА ПОЛНАЯ
                    # =================================================
                        ...
                        if not device.spectrum_active:
                            ...
                            # -----------------------------------------
                            # Нет актуального валидного ПАЕД
                            # -----------------------------------------
    ...
    def handle_timeout_error(self):
        ...
        Правило эксплуатации приборов:

            1...4 последовательных неответа
                -> считаются допустимыми временными сбоями;

            5 последовательных неответов
                -> прибор считается вышедшим из строя;

            после 5 неответов
                -> прибор переводится offline и далее не опрашивается;
                -> дальнейшая работа с ним выполняется через процедуру замены.

        ВАЖНО:
            CRC-ошибка не является неответом прибора.
            Она обрабатывается отдельно в handle_ready_read().
        """
        ...
        # ============================================================
        # 2. ПРОВЕРЯЕМ ИНДЕКС ТЕКУЩЕГО ПРИБОРА
        # ============================================================
        ...
        # ============================================================
        # 4. ОПРЕДЕЛЯЕМ КОМАНДУ, КОТОРАЯ НЕ ПОЛУЧИЛА ОТВЕТ
        # ============================================================

        mode = self.select_mode()

        # ============================================================
        # 5. ЕСЛИ НЕ ПОЛУЧЕН НОВЫЙ RADDose
        # ============================================================
        #
        # Старый PAED больше нельзя использовать для принятия решения
        # о запуске нового StartSpectre.
        #
        # Это особенно важно для заполненной цистерны:
        #
        #   старый PAED <= 50
        #       +
        #   новый RadDose не получен
        #
        # не должны привести к запуску спектра по старому измерению.
        #
        # Для Temperature этого не делаем.
        #
        # Для GetSpectre также не делаем, потому что согласно принятой
        # в п.11 логике один пропущенный сегмент GetSpectre допустим
        # и не прерывает текущий спектральный цикл.
        # ============================================================
        ...
        # ============================================================
        # 6. УВЕЛИЧИВАЕМ СЧЁТЧИК ПОСЛЕДОВАТЕЛЬНЫХ НЕОТВЕТОВ
        # ============================================================

        device.no_answer_count += 1

        # ============================================================
        # 7. ОБНОВЛЯЕМ СОСТОЯНИЕ СВЯЗИ В GUI
        # ============================================================
        ...
        # ============================================================
        # 8. ПЯТЬ ПОСЛЕДОВАТЕЛЬНЫХ НЕОТВЕТОВ
        # ============================================================

        if device.no_answer_count >= 5:

            # --------------------------------------------------------
            # Событие исчезновения создаём только один раз.
            # --------------------------------------------------------
            ...
            if device.is_online:
                ...
                # ----------------------------------------------------
                # Если в этот момент шёл спектральный цикл,
                # продолжать его уже нельзя.
                # ----------------------------------------------------

                device.spectrum_active = False

                # ----------------------------------------------------
                # Сбрасываем счётчик попыток запуска спектра.
                # ----------------------------------------------------

                device.start_spectre_retries = 0

                # ----------------------------------------------------
                # Сообщаем App о подтверждённой потере прибора.
                # ----------------------------------------------------
                ...
                # ----------------------------------------------------
                # Системное событие создаётся один раз именно здесь.
                # ----------------------------------------------------
    ...
    def handle_ready_read(self):
        ...
        ВАЖНО ДЛЯ ЛОГИКИ П.27:

            no_answer_count считает именно ПОСЛЕДОВАТЕЛЬНЫЕ НЕОТВЕТЫ
            прибора.

            Поэтому получение полного пакета от прибора, даже если
            этот пакет имеет ошибочный CRC, разрывает последовательность
            неответов:

                device.no_answer_count = 0

            При этом пакет с ошибочным CRC остаётся недостоверным
            и никогда не передаётся дальше как результат измерения.

            Одиночная ошибка CRC или ошибка одного GetSpectre
            НЕ прекращает текущий спектральный цикл.

            Это соответствует принятой ранее логике:
            отдельный пропущенный/повреждённый GetSpectre допустим,
            а спектральный цикл продолжается до достижения требуемого
            времени накопления.

            Только 5 последовательных реальных неответов, обработанных
            handle_timeout_error(), окончательно переводят прибор offline.
        """
        ...
        try:
            ...
            if start_index == -1:
                ...
                #
                # ВАЖНО:
                # здесь error_timer НЕ останавливаем.
                #
                # Пока полного пакета нет, мы ещё не можем утверждать,
                # что прибор действительно ответил.
                # --------------------------------------------------------
            ...
            # После получения полного ожидаемого пакета остаток текущего
            # обмена нам не нужен.
            #
            # Следующий запрос начнётся с чистого RX-буфера.

            self.rx_buffer.clear()

            # ============================================================
            # 8. ПОЛНЫЙ ПАКЕТ ПОЛУЧЕН — ОСТАНАВЛИВАЕМ TIMEOUT
            # ============================================================
            ...
            if not self.verify_crc(
                ...
                # --------------------------------------------------------
                # КЛЮЧЕВОЕ ИЗМЕНЕНИЕ П.27
                # --------------------------------------------------------
                #
                # Полный пакет физически пришёл от линии.
                #
                # Следовательно, это уже НЕ "прибор не ответил".
                #
                # CRC-пакет нельзя использовать как измерение,
                # но он разрывает последовательность timeout.
                # --------------------------------------------------------

                device.no_answer_count = 0

                # --------------------------------------------------------
                # Для RadDose повреждённый пакет нельзя использовать
                # для принятия решения о запуске спектра.
                #
                # Поэтому старый управляющий PAED инвалидируем.
                # --------------------------------------------------------
                ...
                #     device.spectrum_active = False
                #
                # для StartSpectre/GetSpectre.
                #
                # Одиночная CRC-ошибка не является основанием
                # уничтожать весь текущий спектральный цикл.
                # --------------------------------------------------------
            ...
            #
            # Корректный CRC сам по себе ещё не доказывает,
            # что пакет принадлежит текущему DeviceInfo.
            # ============================================================
            ...
            # код 0x8B.
            #
            # Более глубокую структуру спектрального протокола
            ...
            # ------------------------------------------------------------
            # Полностью корректный ответ однозначно разрывает цепочку
            ...
            #
            # В штатной работе после 5 timeout он больше не опрашивается,
            # поэтому это присваивание относится прежде всего к обычному
            # успешному обмену до подтверждённой поломки.
            # ------------------------------------------------------------
            ...
            if mode == "StartSpectre":
                ...
                #
                # Само значение поля без документации ECOTEST
                # не изменяем.
                # --------------------------------------------------------
                ...
                # --------------------------------------------------------
                # StartSpectre успешно подтверждён прибором.
                # --------------------------------------------------------
            ...
            if mode == "GetSpectre":
                ...
                try:
                    ...
                    # ----------------------------------------------------
                    #
                    # Один повреждённый сегмент GetSpectre
                    # пропускаем.
                    #
                    # НЕ выполняем:
                    #
                    #     device.spectrum_active = False
                    #
                    # Следующий цикл polling сможет снова запросить
                    # GetSpectre.
                    #
                    # Уже накопленные корректные сегменты спектра
                    # сохраняются.
                    # ----------------------------------------------------
        ...
        # ================================================================
            ...
            # ------------------------------------------------------------
            # Любая непредвиденная ошибка текущего ответа не должна
            # останавливать общий цикл polling.
            # ------------------------------------------------------------
    ...
    def _finish_current_poll(self):
        ...
        После ручной остановки системы новый poll_timer
        запускаться не должен.
        """

        # ============================================================
        # 1. ОСТАНАВЛИВАЕМ TIMEOUT ТЕКУЩЕГО ЗАПРОСА
        # ============================================================
        ...
        # ============================================================
        # 4. ПРОВЕРЯЕМ, РАЗРЕШЕНО ЛИ ПРОДОЛЖЕНИЕ POLLING
        # ============================================================
        #
        # Это ключевая защита ручного Stop.
        #
        # Возможна ситуация:
        #
        #     запрос отправлен
        ...
        #     оператор нажал Stop
        #         ->
        #     stop_all() установил running = False
        #         ->
        #     ранее поставленный в очередь обработчик ответа
        #     всё же дошёл до _finish_current_poll()
        #
        # Старый вариант безусловно запускал poll_timer и тем самым
        # мог фактически восстановить polling после команды Stop.
        # ============================================================
        ...
        # ============================================================
        # 5. ПЛАНИРУЕМ СЛЕДУЮЩИЙ ШАГ ОПРОСА
        # ============================================================
    ...
    @Slot(str)
    def finish_spectrum_cycle(self, serial_number: str):
        """
        Сообщает DeviceManager, что один полный спектральный
        цикл данного прибора полностью завершён.

        После этого следующий цикл для заполненной цистерны
        снова сможет пройти:
        ...
        ВАЖНО:
            этот метод выполняется в потоке DeviceManager.
        """
        ...
        for device in self.devices:
            ...
            # Завершаем текущий спектральный цикл
            # с точки зрения управляющей логики DeviceManager.
            device.spectrum_active = False

            # Новая серия StartSpectre должна начинаться
            # с чистого счётчика попыток.
            device.start_spectre_retries = 0

            self.device_info.emit(
                (
                    "Спектральний цикл завершено для "
                    f"SN {serial_number}."
                )
            )
