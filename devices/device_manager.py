...
################################################ БЛОК ЦИКЛИЧЕСКОГО ОПРОСА ПРИБОРОВ ######################################################  
    ...
    def handle_ready_read(self):
        ...
        Основные задачи метода:
            ...
            6. Проверить код ответа с учётом протокола ECOTEST.
        ...
        ВАЖНО:
            ...
            Получение полного пакета, даже с ошибочным CRC, означает,
            что прибор физически ответил, поэтому последовательность
            timeout разрывается.

            CRC-пакет при этом не используется как результат измерения.

            Для RadDose протокол имеет особенность:

                код запроса  = 0x00
                код ответа   = 0x01

            Для остальных используемых команд:

                Temperature:
                    запрос 0x08 -> ответ 0x08

                StartSpectre:
                    запрос 0x8B -> ответ 0x8B
            ...
            Поэтому прямое сравнение:
                ...
                self.last_command.data[4] == packet_data[4]

            для всех команд использовать нельзя.
        """
        ...
        try:
            ...
            if start_index == -1:
                ...
                #
                # Если последний байт 0x55, сохраняем его, потому что
            ...
            if not self.verify_crc(
                ...
                # Полный пакет физически получен.
                # Следовательно, это не является no-response.

                device.no_answer_count = 0

                mode = self.select_mode()

                # Повреждённый RadDose нельзя использовать для принятия
                # решения о запуске спектра.
            ...
            # ============================================================
            # 11. ПРОВЕРЯЕМ КОД ОТВЕТА
            # ============================================================
            #
            # ВАЖНО:
            #
            # Для RadDose код запроса и код ответа по протоколу РАЗНЫЕ:
            #
            #     запрос: 0x00
            #     ответ:  0x01
            #
            # Для Temperature и используемых спектральных команд код
            # ответа совпадает с кодом запроса.
            #
            # Поэтому здесь нельзя просто сравнивать:
            #
            #     self.last_command.data[4] == packet_data[4]
            #
            # Сначала определяем ожидаемый код ответа.
            # ============================================================
            ...
            if request_command_code == 0x00:
                ...
                expected_response_code = 0x01

            else:

                # --------------------------------------------------------
                # Для используемых здесь остальных команд:
                #
                # Temperature:
                #     0x08 -> 0x08
                #
                # StartSpectre/GetSpectre:
                #     0x8B -> 0x8B
                # --------------------------------------------------------
            ...
            if (
                ...
                self.device_error.emit(
                    ...
                    (
                        ...
                        f"очікувався код відповіді "
                        f"0x{expected_response_code:02X}, "
                        f"отримано "
                ...
                self.system_event.emit(
                    ...
                    (
                        "Невідповідність коду відповіді: "
                        f"код запиту "
                        f"0x{request_command_code:02X}, "
                        f"очікуваний код відповіді "
                        f"0x{expected_response_code:02X}, "
                        f"отриманий "
                        f"0x{received_command_code:02X}."
                    )
            ...
            # Корректный ответ разрывает последовательность timeout.
            ...
            if mode == "GetSpectre":
                ...
                try:
                    ...
                    # Один повреждённый сегмент GetSpectre пропускаем.
                    # Активный спектральный цикл не уничтожаем.
        ...
        # ================================================================
            ...
            # Любая непредвиденная ошибка текущего ответа
            # не должна останавливать общий polling.
