...
class DatabaseManager:
    ...
    def _create_tables_if_not_exist(self):
        ...
        и выполняет безопасную миграцию существующих таблиц.

        ================================================================
        measurements_wall
        ================================================================

        Для настенных детекторов добавляются:

            temperature_valid INTEGER

                1 -> сохранённая температура подтверждена
                    корректным ответом прибора;

                0 -> текущее PAED получено, но актуальная температура
                    не подтверждена;

                NULL -> старые записи БД, созданные до введения
                        этого поля.

            temperature_timestamp DATETIME

                Время последнего успешного получения температуры.

                При ошибке нового температурного измерения числовое
                значение last_temperature не уничтожается, но:

                    temperature_valid = 0

                а temperature_timestamp продолжает указывать,
                когда температура была реально получена последний раз.

        ================================================================
        measurements_cistern
        ================================================================

        Сохраняются изменения п.17:
        ...
        Существующие записи не удаляются и не переписываются.
        """
        ...
        # ============================================================
        #
        # Для новой БД сразу создаётся актуальная структура.
        #
        # temperature остаётся REAL NOT NULL для совместимости
        # с существующей схемой.
        #
        # Смысл значения определяется отдельным
        # temperature_valid.
        # ============================================================
        ...
        # ============================================================
        # 3. МИГРАЦИЯ СУЩЕСТВУЮЩЕЙ measurements_wall
        # ============================================================
        ...
        cursor.execute(
            "PRAGMA table_info(measurements_wall)"
        )

        wall_table_info = cursor.fetchall()

        wall_existing_columns = {
            row["name"]
            for row in wall_table_info
        }

        wall_required_columns = {
            "temperature_valid": "INTEGER",
            "temperature_timestamp": "DATETIME",
        }
        ...
        try:
            ...
            for column_name, column_type in (
                wall_required_columns.items()
            ):

                if column_name in wall_existing_columns:
                    continue

                cursor.execute(
                    f"""
                    ALTER TABLE measurements_wall
        ...
        # ============================================================
        # 4. ТАБЛИЦА ИЗМЕРЕНИЙ ЦИСТЕРН
        # ============================================================
        #
        # Полная структура после п.17.
        # ============================================================
        ...
        # ============================================================
        # 5. МИГРАЦИЯ СУЩЕСТВУЮЩЕЙ measurements_cistern
        # ============================================================
        ...
        cistern_table_info = cursor.fetchall()

        cistern_existing_columns = {
            row["name"]
            for row in cistern_table_info
        }

        cistern_required_columns = {
            "activity_upper": "TEXT",
            "concentration_upper": "TEXT",
            "result_meta": "TEXT",
            "ready_to_drain": "INTEGER",
        ...
        try:
            ...
            for column_name, column_type in (
                cistern_required_columns.items()
            ):

                if column_name in cistern_existing_columns:
                    continue
    ...
    def buffer_wall_measurement(
        ...
        """
        Добавляет измерение настенного детектора в буфер.

        В буфере хранится только ПОСЛЕДНЕЕ измерение
        ...
        paed
            Текущее значение ПАЕД.

        temperature
            Последнее известное числовое значение температуры.

            Оно может быть старым, если новое температурное
            измерение оказалось невалидным.

        temperature_valid
            True:
                на момент формирования текущей записи имеется
                подтверждённое корректное значение температуры.

            False:
                актуальная температура не подтверждена.

                При этом поле temperature может содержать последнее
                ранее успешно полученное значение.

        temperature_timestamp
            Время ПОСЛЕДНЕГО успешного измерения температуры.

            Если температура ещё ни разу не была успешно получена,
            значение должно быть None.

        low_status
            Состояние низкочувствительного канала.

        high_status
            Состояние высокочувствительного канала.

        valid
            Валидность текущего измерения PAED.

        ================================================================
        ВАЖНО
        ================================================================

        Буфер хранится в виде dict, а не tuple.

        Это исключает зависимость от номера позиции поля и делает
        дальнейшую запись в БД безопаснее и понятнее.
        """

        # ============================================================
        # 1. ВРЕМЯ ТЕКУЩЕГО PAED-ИЗМЕРЕНИЯ
        # ============================================================
        ...
        # ============================================================
        # 2. НОРМАЛИЗУЕМ temperature_valid
        # ============================================================

        temperature_valid = (
            1
            if bool(temperature_valid)
            else 0
        )

        # ============================================================
        # 3. СОХРАНЯЕМ ПОСЛЕДНИЙ СНИМОК ПРИБОРА
        # ============================================================
        #
        # Новое измерение того же device_id заменяет предыдущее.
        #
        # Это соответствует существующей архитектуре:
        #
        #     один последний снимок на прибор
        #     между периодическими flush.
        # ============================================================

        self.wall_buffer[device_id] = {
            "timestamp": timestamp,
            "paed": paed,
    ...
    def _flush_wall_buffer(self):
        """
        Записывает буферизованные измерения настенных детекторов
        в таблицу measurements_wall.

        ================================================================
        ОСНОВНОЙ ПРИНЦИП
        ================================================================

        В wall_buffer хранится только последнее измерение
        для каждого device_id.

        Перед началом записи создаётся снимок текущего буфера.

        После успешного COMMIT из wall_buffer удаляется только та запись,
        которая всё ещё является именно тем объектом, который был сохранён.

        Если во время записи в БД для того же device_id пришло новое
        измерение и заменило старый dict, новая запись остаётся в буфере.

        ================================================================
        ОБРАБОТКА ОШИБОК
        ================================================================

        При любой ошибке INSERT или COMMIT:

            - выполняется rollback;
            - wall_buffer не очищается;
            - исключение передаётся выше.

        Таким образом данные остаются в памяти и могут быть повторно
        сохранены при следующем вызове flush.
        """

        # ============================================================
        # 1. ЕСЛИ БУФЕР ПУСТ — НИЧЕГО НЕ ДЕЛАЕМ
        # ============================================================
        ...
        # ============================================================
        # 2. ФИКСИРУЕМ СНИМОК БУФЕРА
        # ============================================================
        #
        # list(...) создаёт отдельный список пар:
        #
        #     (device_id, record)
        #
        # Сам record остаётся тем же объектом dict.
        # Это понадобится после COMMIT для проверки:
        #
        #     current_record is saved_record
        #
        # ============================================================

        records_to_flush = list(
            self.wall_buffer.items()
        )

        # ============================================================
        # 3. ПОЛУЧАЕМ СОЕДИНЕНИЕ
        # ============================================================
        ...
        try:
            ...
            # ========================================================
            # 4. ЗАПИСЫВАЕМ ВСЕ ЗАФИКСИРОВАННЫЕ ЗАПИСИ
            # ========================================================
            ...
            for device_id, record in records_to_flush:
                ...
                cursor.execute(
                    ...
                    (
                        ...
                        record["paed"],
                        record["temperature"],
                        record["temperature_valid"],
                        record["temperature_timestamp"],
                        record["low_status"],
                        record["high_status"],
                        record["valid"],
                    )
            ...
            # ========================================================
            # 6. ПРИ ОШИБКЕ ОТКАТЫВАЕМ ВСЮ ТРАНЗАКЦИЮ
            # ========================================================
            ...
            # Буфер НЕ очищаем.
            #
            # Исключение передаём выше, чтобы ошибка не была скрыта.
            # --------------------------------------------------------

            raise

        # ============================================================
        # 7. УДАЛЯЕМ ИЗ БУФЕРА ТОЛЬКО УСПЕШНО СОХРАНЁННЫЕ СНИМКИ
        # ============================================================
        #
        # Если за время выполнения INSERT/COMMIT пришло более новое
        # измерение этого же прибора, то:
        #
        #     self.wall_buffer[device_id]
        #
        # уже будет ссылаться на другой dict.
        #
        # Такое новое измерение нельзя удалять.
        # ============================================================

        for device_id, saved_record in records_to_flush:

            current_record = self.wall_buffer.get(
                device_id
            )

            if current_record is saved_record:

                del self.wall_buffer[device_id]
    ...
    def save_system_event(self, device_id, event_type, description):
        ...
        Параметры:
            device_id
                ...
                Может быть None для событий, которые относятся
                ко всей программе, например:
        ...
        Запись системного события выполняется отдельной
        контролируемой транзакцией:

            INSERT
            -> commit

        При любой ошибке SQLite выполняется rollback(),
        после чего исключение передаётся вызывающему коду.

        Исключение здесь намеренно НЕ скрывается.

        Конкретный вызывающий код должен решать, является ли
        невозможность записать системное событие критической
        для выполняемой операции.
        """

        # ============================================================
        # 1. ФОРМИРУЕМ ВРЕМЯ СОБЫТИЯ
        # ============================================================
        ...
        try:
            ...
            # ========================================================
            # 6. ПРИ ОШИБКЕ ОБЯЗАТЕЛЬНО ОТКАТЫВАЕМ ТРАНЗАКЦИЮ
            # ========================================================
            #
            # Это особенно важно для длительной работы программы:
            # после неудачного INSERT соединение не должно оставаться
            # в незавершённом транзакционном состоянии.
            # ========================================================
            ...
            try:
                ...
                # Ошибка самого rollback не должна скрыть
                # исходную ошибку записи события.
                pass

            # ========================================================
            # 7. ПЕРЕДАЁМ ИСХОДНУЮ ОШИБКУ ВЫШЕ
            # ========================================================
