...
class DeviceCardBarrel(QWidget):
    ...
    def reset_spectrum(self):
        ...
        Этот метод выполняет только технический сброс состояния спектра.

        Он НЕ записывает системное событие "spectrum_reset", потому что
        reset_spectrum() вызывается в разных ситуациях:

            - после нормального успешного спектрального цикла;
            - после отклонённого расчёта;
            - при заполнении цистерны;
            - при опорожнении цистерны;
            - при других служебных сбросах.

        Причина сброса должна журналироваться там, где эта причина
        действительно известна.
        """

        # ============================================================
        # 1. НАКОПЛЕННЫЙ СПЕКТР
        # ============================================================

        self.spectrum_buffer = [0] * 1023

        # ============================================================
        # 2. ДИАГНОСТИЧЕСКИЙ СЧЁТЧИК ПАКЕТОВ GetSpectre
        # ============================================================

        self.spectrum_counter = 0

        # ============================================================
        # 3. ДИАГНОСТИЧЕСКОЕ ВРЕМЯ ПРИБОРА
        # ============================================================
        ...
        # ============================================================
        # 4. ВРЕМЯ НАЧАЛА СПЕКТРАЛЬНОГО ЦИКЛА
        # ============================================================
        #
        # Новый цикл ещё не подтверждён успешным StartSpectre.
        # ============================================================

        self.spectrum_start_monotonic = None

        # ============================================================
        # 5. ФАКТИЧЕСКОЕ ВРЕМЯ ЗАВЕРШЁННОГО ЦИКЛА
        # ============================================================
        #
        # Старое значение больше не относится к следующему циклу.
        # ============================================================

        self.spectrum_elapsed_time = 0.0

        # ============================================================
        # 6. ЛОКАЛЬНЫЙ ФЛАГ СПЕКТРАЛЬНОГО РЕЖИМА
        # ============================================================
...
class App(QObject):
    ...
    def load_ui(self):
        """
        Загружает основной интерфейс программы.

        Дополнительно ограничивает размер оперативного GUI-журнала
        textEdit, чтобы при непрерывной работе 24/7 количество
        отображаемых сообщений не росло бесконечно.

        Это ограничение касается только текста в GUI.
        Данные измерений и системные события в БД не затрагиваются.
        """

        # ============================================================
        # 1. СОЗДАЁМ ЗАГРУЗЧИК UI
        # ============================================================

        loader = QUiLoader()

        # ============================================================
        # 2. ОТКРЫВАЕМ ФАЙЛ ГЛАВНОГО ИНТЕРФЕЙСА
        # ============================================================
        ...
        if not ui_file.open(QFile.ReadOnly):
            ...
            raise RuntimeError(
                "Не вдалося відкрити main_window_form.ui"
            )
        ...
        if self.ui is None:
            ...
            raise RuntimeError(
                "Не вдалося завантажити main_window.ui"
            )

        # ============================================================
        # 5. ОГРАНИЧИВАЕМ РАЗМЕР GUI-ЖУРНАЛА
        # ============================================================
        #
        # QTextEdit по умолчанию способен бесконечно накапливать
        # добавляемый через append() текст.
        #
        # Для программы, работающей 24/7, это означает постепенный
        # рост потребления оперативной памяти.
        #
        # Оставляем последние 5000 текстовых блоков.
        # Старые строки автоматически удаляются самим Qt.
        #
        # ВАЖНО:
        # это НЕ удаляет записи из SQLite и никак не влияет
        # на историю измерений или системных событий.
        # ============================================================

        if hasattr(self.ui, "textEdit"):

            self.ui.textEdit.document().setMaximumBlockCount(
                100
            )
    ...
    def _schedule_bridge_restart(self):
        """
        Планирует автоматический перезапуск ModBusBridgeService.

        Перезапуск разрешён ТОЛЬКО если оператор по-прежнему
        запрашивает активное соединение с PLC:

            self._plc_connection_requested == True

        Ручной Disconnect устанавливает этот флаг в False,
        поэтому после ручного отключения Bridge автоматически
        запускаться не будет.

        Для защиты от быстрого бесконечного цикла падений используется
        задержка 5 секунд между попытками запуска.

        Одновременно может существовать только один ожидающий
        таймер автоматического восстановления.
        """

        # ============================================================
        # 1. ПРОВЕРЯЕМ, НУЖНО ЛИ ВООБЩЕ ВОССТАНАВЛИВАТЬ BRIDGE
        # ============================================================

        if not getattr(
            self,
            "_plc_connection_requested",
            False
        ):
            return

        # ============================================================
        # 2. НЕ СОЗДАЁМ НЕСКОЛЬКО ТАЙМЕРОВ
        # ============================================================
        #
        # QProcess при аварии может сначала выдать errorOccurred,
        # а затем finished.
        #
        # Поэтому несколько обработчиков не должны создавать
        # несколько параллельных попыток запуска Bridge.
        # ============================================================

        if not hasattr(
            self,
            "_bridge_restart_timer"
        ):

            self._bridge_restart_timer = QTimer(
                self
            )

            self._bridge_restart_timer.setSingleShot(
                True
            )

            self._bridge_restart_timer.timeout.connect(
                self._restart_bridge_after_failure
            )

        if self._bridge_restart_timer.isActive():
            return

        # ============================================================
        # 3. ЗАПУСКАЕМ ОТЛОЖЕННУЮ ПОПЫТКУ
        # ============================================================

        self.ui.textEdit.append(
            (
                "[Bridge] Автоматичне відновлення "
                "буде виконано через 5 секунд."
            )
        )

        self._bridge_restart_timer.start(
            5000
        )


    def _restart_bridge_after_failure(self):
        """
        Выполняет одну попытку автоматического восстановления
        ModBusBridgeService после его аварийного завершения.

        Если запуск снова не удался, новая попытка планируется
        через _schedule_bridge_restart().

        Если Bridge успешно запущен, восстанавливаем локальное
        соединение Python -> Bridge.

        После успешного TCP reconnect существующий
        on_bridge_connected():

            - выполнит _resync_bridge_state();
            - запросит актуальный fullness;
            - восстановит контроль PLC из п.25.
        """

        # ============================================================
        # 1. ОПЕРАТОР МОГ ЗА ЭТИ 5 СЕКУНД НАЖАТЬ DISCONNECT
        # ============================================================
        ...
        # ============================================================
        # 2. ЗАЩИТА ОТ ЗАПУСКА ВТОРОГО BRIDGE
        # ============================================================
        ...
        # ============================================================
        # 3. БЕРЁМ ПАРАМЕТРЫ PLC
        # ============================================================
        #
        # Используем текущие значения интерфейса.
        #
        # Во время активного сеанса именно они являются параметрами,
        # с которыми был запущен Bridge.
        # ============================================================
        ...
        if not ip:
            ...
            self.ui.textEdit.append(
                (
                    "[Bridge] Автоматичне відновлення неможливе: "
                    "IP-адреса PLC порожня."
                )
            )
        ...
        # ============================================================
        # 5. ПЫТАЕМСЯ ЗАПУСТИТЬ BRIDGE
        # ============================================================

        self.ui.textEdit.append(
            "[Bridge] Спроба автоматичного перезапуску..."
        )
        ...
        if not self.start_bridge(
            ...
            # --------------------------------------------------------
            # Bridge не запустился.
            #
            # Сеанс НЕ отменяем.
            # Через 5 секунд пробуем снова.
            # --------------------------------------------------------

            self.ui.textEdit.append(
                (
                    "[Bridge] Автоматичний перезапуск "
                    "не вдався."
                )
            )
        ...
        # ============================================================
        # 6. ПРОЦЕСС УСПЕШНО ВОССТАНОВЛЕН
        # ============================================================

        self.ui.textEdit.append(
            "[Bridge] Процес автоматично відновлено."
        )

        # ============================================================
        # 7. ВОССТАНАВЛИВАЕМ PYTHON -> BRIDGE
        # ============================================================
        #
        # Внутренний reconnect ModBusBridgeClient мог уже ожидать
        # появления TCP-сервера.
        #
        # Дополнительный connect_to_bridge() здесь нужен также для
        # сценария, когда предыдущий процесс вообще не успел
        ...
        try:
            ...
            if not self.modbus_client.is_connected():
                ...
                self.modbus_client.connect_to_bridge()

        except Exception as e:

            # --------------------------------------------------------
            # Сам процесс уже работает.
            #
            # Ошибка непосредственной попытки TCP-подключения
            # не является причиной снова перезапускать процесс:
            # ModBusBridgeClient продолжит собственный reconnect.
            # --------------------------------------------------------

            self.ui.textEdit.append(
                (
                    "[Bridge] Процес відновлено, але "
                    "TCP-з'єднання поки не встановлено: "
                    f"{e}"
                )
            )
    ...
    def on_bridge_finished(
        self,
        exit_code,
        exit_status
    ):
        """
        Обрабатывает завершение процесса ModBusBridgeService.

        Есть два принципиально разных случая:

            1. Оператор сам отключил PLC.
            _plc_connection_requested == False

            Автоматический restart НЕ выполняется.

            2. Активный PLC-сеанс был запрошен, но Bridge завершился.
            _plc_connection_requested == True

            Сеанс сохраняется и Bridge автоматически
            перезапускается.

        ВАЖНО:

        Здесь НЕ вызываем modbus_client.disconnect() при аварии.

        disconnect() является намеренным отключением клиента
        и запрещает его автоматический reconnect.
        """

        # ============================================================
        # 1. ФОРМИРУЕМ ПРИЧИНУ ЗАВЕРШЕНИЯ
        # ============================================================
        ...
        self.ui.textEdit.append(
            (
                "[Bridge] Процес завершено "
                f"з кодом {exit_code} ({status_text})"
            )
        )

        # ============================================================
        # 2. ЭТОТ QPROCESS БОЛЬШЕ НЕ РАБОТАЕТ
        # ============================================================

        self.bridge_process = None

        # ============================================================
        # 3. СОСТОЯНИЕ PLC БОЛЬШЕ НЕ ПОДТВЕРЖДЕНО
        # ============================================================

        self.update_plc_indicators(
            plc_connected=False,
            bridge_connected=False
        )

        # ============================================================
        # 4. ПРОВЕРЯЕМ НАМЕРЕНИЕ ОПЕРАТОРА
        # ============================================================

        connection_requested = getattr(
            self,
            "_plc_connection_requested",
            False
        )
        ...
        # ============================================================
        # 5. РУЧНОЕ / ШТАТНОЕ ОТКЛЮЧЕНИЕ
        # ============================================================
        #
        # on_connect_plc_clicked() ДО stop_bridge()
        # устанавливает _plc_connection_requested=False.
        #
        # Поэтому finished от terminate()/kill() не сможет
        # случайно снова запустить Bridge.
        # ============================================================

        if not connection_requested:

            if self.btn_connect_plc is not None:

                self.btn_connect_plc.setText(
                    "Connect PLC"
                )
        ...
        self.ui.textEdit.append(
            (
                "[Bridge] Процес завершився під час "
                "активного PLC-сеансу."
            )
        )
        ...
        # ============================================================
        # 7. НЕ ОТКЛЮЧАЕМ ModBusBridgeClient
        # ============================================================
        #
        # Его reconnect-механизм должен оставаться разрешённым.
        # После появления нового Bridge он сможет восстановить
        # TCP-соединение.
        # ============================================================

        # ============================================================
        # 8. ПЛАНИРУЕМ ПЕРЕЗАПУСК
        # ============================================================
    ...
    def on_bridge_error(self, error):
        """
        Обрабатывает QProcess.errorOccurred.

        Метод НЕ отменяет активный PLC-сеанс.

        Основной автоматический restart после падения выполняется
        через on_bridge_finished().

        Исключение — FailedToStart:
        процесс вообще не стартовал, поэтому отдельно планируем
        следующую попытку запуска.
        """

        # ============================================================
        # 1. ПРЕОБРАЗУЕМ QPROCESS ERROR В ТЕКСТ
        # ============================================================
        ...
        # ============================================================
        # 2. СОСТОЯНИЕ СВЯЗИ СЧИТАЕМ НЕПОДТВЕРЖДЁННЫМ
        # ============================================================
        ...
        # ============================================================
        # 3. НЕ СБРАСЫВАЕМ _plc_connection_requested
        # ============================================================
        #
        # Это принципиальное отличие от старой реализации.
        #
        # Ошибка процесса не означает, что оператор запросил
        # отключение системы.
        # ============================================================
        ...
        # ============================================================
        #
        # Если процесс вообще не смог стартовать, finished()
        # может не дать нам обычный путь восстановления.
        #
        # Поэтому планируем следующую попытку здесь.
        # ============================================================
        ...
        #
        # finished() очистит bridge_process и поставит restart.
        #
        # Это предотвращает двойной restart одного и того же
        # аварийного процесса.
        # ============================================================
    ...
    def on_connect_plc_clicked(self):
        """
        Обрабатывает кнопку Connect PLC / Disconnect PLC.

        CONNECT:
            - пользователь запрашивает постоянный PLC-сеанс;
            - запускается Bridge;
            - устанавливается Python -> Bridge;
            - последующие аварии Bridge восстанавливаются автоматически.

        DISCONNECT:
            - пользователь явно отменяет PLC-сеанс;
            - автоматический restart запрещается;
            - ожидающий restart отменяется;
            - Python отключается от Bridge;
            - Bridge завершается.
        """

        # ============================================================
        # 1. ОПРЕДЕЛЯЕМ ТЕКУЩЕЕ СОСТОЯНИЕ СЕАНСА
        # ============================================================
        ...
        if (
            ...
            or bridge_process_running
            or bridge_socket_connected
        ):

            self.ui.textEdit.append(
                "[PLC] Відключення від PLC..."
            )

            # --------------------------------------------------------
            # САМОЕ ВАЖНОЕ:
            #
            # сначала отменяем намерение поддерживать соединение.
            #
            # После этого ни finished(), ни errorOccurred,
            # ни отложенный restart не смогут восстановить Bridge.
            # --------------------------------------------------------

            self._plc_connection_requested = False

            # --------------------------------------------------------
            # Отменяем уже запланированный restart.
            # --------------------------------------------------------

            restart_timer = getattr(
                self,
                "_bridge_restart_timer",
                None
            )
            ...
            # --------------------------------------------------------
            # Намеренно отключаем Python -> Bridge.
            # --------------------------------------------------------
        ...
        if not ip:
            ...
            self.ui.textEdit.append(
                "Помилка: IP-адреса не може бути порожньою"
            )
        ...
        # ============================================================
        # 5. ФИКСИРУЕМ НАМЕРЕНИЕ ОПЕРАТОРА
        # ============================================================
        ...
        if not self.start_bridge(
            ...
            # --------------------------------------------------------
            # В п.26 это больше НЕ является окончанием сеанса.
            #
            # Пользователь запросил связь с PLC.
            # Поэтому продолжаем попытки автоматически.
            # --------------------------------------------------------

            self.ui.textEdit.append(
                (
                    "[Bridge] Початковий запуск не вдався. "
                    "Автоматичне відновлення продовжується."
                )
            )
        ...
        # ============================================================
        # 7. ПОДКЛЮЧАЕМ PYTHON К BRIDGE
        # ============================================================

        self.ui.textEdit.append(
            "[Bridge] Спроба підключення до Bridge..."
        )
        ...
        # ============================================================
        # 8. РЕЗУЛЬТАТ ПЕРВИЧНОГО TCP-ПОДКЛЮЧЕНИЯ
        # ============================================================

        if connected:

            self.ui.textEdit.append(
                "[Bridge] Підключення до Bridge успішне"
            )

            # connected дополнительно вызовет on_bridge_connected().

        else:

            self.ui.textEdit.append(
                (
                    "[Bridge] Початкове підключення не вдалося. "
                    "Автоматичне відновлення з'єднання продовжується."
                )
            )
    ...
    def on_bridge_connected(self):
        """
        Обрабатывает установление TCP-соединения Python -> Bridge.

        ВАЖНО:

        Соединение с Bridge ещё НЕ означает, что Bridge подключён к PLC.

        PLC считается доступным только после получения свежих
        результатов чтения fullness.
        """
        ...
        self.ui.textEdit.append(
            "[Bridge] З'єднання з Bridge встановлено"
        )

        # ============================================================
        # 1. BRIDGE ДОСТУПЕН, PLC ПОКА НЕ ПОДТВЕРЖДЁН
        # ============================================================
        ...
        # ============================================================
        # 2. СОЗДАЁМ МОНИТОР АКТУАЛЬНОСТИ PLC
        # ============================================================

        if not hasattr(
            self,
            "_plc_fullness_last_seen"
        ):

            self._plc_fullness_last_seen = {}

        self._plc_stale_positions = set(
            range(1, 10)
        )

        if not hasattr(
            self,
            "_plc_fullness_monitor_timer"
        ):

            self._plc_fullness_monitor_timer = QTimer(
                self
            )

            self._plc_fullness_monitor_timer.setInterval(
                10000
            )

            self._plc_fullness_monitor_timer.timeout.connect(
                self._check_plc_fullness_timeout
            )

        if not self._plc_fullness_monitor_timer.isActive():

            self._plc_fullness_monitor_timer.start()

        # ============================================================
        # 3. ДО ПЕРВОГО ОТВЕТА PLC FULLNESS НЕДОСТОВЕРНА
        # ============================================================
        ...
        # ============================================================
        # 4. РЕСИНХРОНИЗИРУЕМ PYTHON -> BRIDGE
        # ============================================================

        self._resync_bridge_state()

        # ============================================================
        # 5. ПРОСИМ НЕМЕДЛЕННО ПРОЧИТАТЬ FULLNESS
        # ============================================================
        #
        # Не ждём до 20 секунд очередного периодического цикла Bridge.
        # ============================================================

        try:

            if not self.modbus_client.request_fullness():

                self.ui.textEdit.append(
                    (
                        "[PLC] Не вдалося відправити "
                        "початковий запит fullness."
                    )
                )
    ...
    def on_bridge_disconnected(self):
        """
        Обрабатывает разрыв TCP-соединения Python -> Bridge.

        Различаются две ситуации:

            1. Ручное отключение пользователя.
            В этом случае соединение не "потеряно" —
            пользователь сам нажал Disconnect PLC.

            2. Неожиданный обрыв связи.
            В этом случае активный сеанс остаётся запрошенным,
            а ModBusBridgeClient продолжает автоматический reconnect.

        Состояние PLC при потере Bridge также считается
        неподтверждённым/отключённым.
        """

        # ============================================================
        # 1. ПРОВЕРЯЕМ, ХОЧЕТ ЛИ ПОЛЬЗОВАТЕЛЬ СОХРАНЯТЬ СОЕДИНЕНИЕ
        # ============================================================
        #
        # Перед ручным Disconnect в on_connect_plc_clicked()
        # устанавливается:
        #
        #     self._plc_connection_requested = False
        #
        # При неожиданном обрыве этот флаг остаётся True.
        # ============================================================
        ...
        # ============================================================
        # 2. СБРАСЫВАЕМ ИНДИКАТОРЫ
        # ============================================================
        #
        # TCP-соединения Python -> Bridge сейчас нет.
        #
        # Следовательно:
        #
        #     Bridge = disconnected
        ...
        if connection_requested:
            ...
            self.ui.textEdit.append(
                "[Bridge] З'єднання з Bridge втрачено"
            )
            ...
            self.ui.textEdit.append(
                "[Bridge] Очікування автоматичного відновлення з'єднання..."
            )
    ...
    def on_bridge_data_received(self, data):
        """
        Обрабатывает ответы ModBusBridgeService.

        Для type == "read" принимаются состояния fullness ZB1...ZB9.

        Каждый успешно подтверждённый ZB получает собственную
        отметку времени актуальности.

        Отсутствующий ZB:
            - НЕ становится empty;
            - старое физическое состояние сохраняется;
            - в DeviceManager передаётся None;
            - спектральный режим для него блокируется.
        """
        ...
        try:
            ...
            for zb in zb_list:
                ...
                if not 1 <= number <= 9:
                    ...
                    self.ui.textEdit.append(
                        (
                            "[Bridge] Помилка: номер "
                            f"ZB{number} поза діапазоном 1..9."
                        )
                    )
                ...
                if (
                    ...
                    self.ui.textEdit.append(
                        (
                            "[Bridge] Помилка: некоректний "
                            f"fullness={fullness} для ZB{number}."
                        )
                    )
            ...
            for number in duplicate_numbers:
                ...
                self.ui.textEdit.append(
                    (
                        f"[Bridge] Помилка: ZB{number} "
                        "повторюється в одному повідомленні. "
                        "Стан проігноровано."
                    )
                )

            # ============================================================
            # 7. ВРЕМЯ ПОДТВЕРЖДЕНИЯ
            # ============================================================
            ...
            for number in validated_states:
            ...
            # ============================================================
            # 8. ФОРМИРУЕМ СНИМОК ДЛЯ DEVICEMANAGER
            # ============================================================
            #
            # Полученные ZB -> True/False.
            #
            # Не полученные в текущем цикле -> None.
            #
            # Это не меняет self.cistern_dict.
            # ============================================================
            ...
            for position in range(1, 10):
                ...
                if position in validated_states:

                    manager_states[position] = (
                        validated_states[position]
                    )

                else:

                    manager_states[position] = None
            ...
            if missing_numbers:
                ...
                self.ui.textEdit.append(
                    (
                        "[Bridge] Увага: PLC не підтвердив "
                        f"стан {missing_text}. "
                        "Попереднє фізичне значення збережено, "
                        "але не використовується для спектрального режиму."
                    )
                )

            # ============================================================
            # 10. НЕТ НИ ОДНОГО УСПЕШНОГО ЧТЕНИЯ
            # ============================================================
            ...
            # ============================================================
            # 11. ОТМЕЧАЕМ ВОССТАНОВИВШИЕСЯ ZB
            # ============================================================

            previous_stale = getattr(
                self,
                "_plc_stale_positions",
                set()
            )
            ...
            if recovered:
                ...
                recovered_text = ", ".join(
                    f"ZB{number}"
                    for number in sorted(recovered)
                )

                self.ui.textEdit.append(
                    (
                        "[PLC] Відновлено актуальне "
                        f"підтвердження fullness: {recovered_text}."
                    )
                )

            self._plc_stale_positions = (
                previous_stale
                - set(validated_states)
            )

            # ============================================================
            # 12. ОБНОВЛЯЕМ ФИЗИЧЕСКОЕ СОСТОЯНИЕ
            # ============================================================
            ...
            # ============================================================
            # 13. НИЧЕГО ФИЗИЧЕСКИ НЕ ИЗМЕНИЛОСЬ
            # ============================================================
            ...
            # ============================================================
            # 14. АТОМАРНО СОХРАНЯЕМ cistern.json
            # ============================================================
            ...
            # ============================================================
            # 16. DEVICEMANAGER УЖЕ ПОЛУЧИЛ manager_states
            # ============================================================
            #
            # Второй emit self.cistern_dict здесь НЕ нужен.
            #
            # Иначе None для недостоверных ZB был бы тут же затёрт
            # старым сохранённым True/False.
            # ============================================================
    ...
    def _check_plc_fullness_timeout(self):
        """
            Контролирует актуальность состояний ZB, получаемых от PLC.

            Bridge опрашивает PLC каждые 20 секунд.

            Если конкретная ZB не получила успешного подтверждения
            в течение 60 секунд, её fullness считается недостоверной.

            Последнее значение self.cistern_dict НЕ изменяется.
        """

        PLC_FULLNESS_TIMEOUT = 60.0

        now = time.monotonic()

        # ============================================================
        # 1. ЕСЛИ BRIDGE НЕ ПОДКЛЮЧЁН
        # ============================================================
        ...
        # ============================================================
        # 2. ВРЕМЯ ПОСЛЕДНЕГО ПОДТВЕРЖДЕНИЯ КАЖДОЙ ZB
        # ============================================================
        ...
        for position in range(1, 10):
            ...
            timestamp = last_seen.get(
                position
            )

            if (
                timestamp is None
                or now - timestamp > PLC_FULLNESS_TIMEOUT
            ):
        ...
        # ============================================================
        # 4. ПЕРЕДАЁМ В DEVICEMANAGER ТОЛЬКО НЕВАЛИДНЫЕ ПОЗИЦИИ
        # ============================================================
        ...
        # ============================================================
        # 5. ЕСЛИ PLC ДАВНО НЕ ПОДТВЕРЖДАЕТ НИ ОДНУ ZB
        # ============================================================

        fresh_exists = any(
            (
                timestamp is not None
                and now - timestamp <= PLC_FULLNESS_TIMEOUT
            )
            for timestamp in last_seen.values()
        )

        self.update_plc_indicators(
            plc_connected=fresh_exists,
            bridge_connected=True
        )

        # ============================================================
        # 6. ЛОГИРУЕМ ПЕРЕХОД В STALE ТОЛЬКО ОДИН РАЗ
        # ============================================================
        ...
        if newly_stale:
            ...
            text = ", ".join(
                f"ZB{position}"
                for position in sorted(newly_stale)
            )

            self.ui.textEdit.append(
                (
                    "[PLC] Втрачено актуальне підтвердження "
                    f"fullness: {text}. "
                    "Спектральний режим для цих ZB заблоковано."
                )
            )
    ...
    def on_device_packet(self, packet):
        ...
        Для ZB обычный RadDose НЕ изменяет:

            ready_to_drain
            isotopes

        Эти поля являются результатом спектрального анализа и должны
        сохраняться до следующего спектрального результата либо
        явного сброса технологического цикла цистерны.

        ВАЖНО ДЛЯ СПЕКТРАЛЬНОГО ЦИКЛА:

            После того как полный спектр признан готовым,
            текущий спектральный цикл должен быть завершён
            независимо от результата calculate_activity().

            Если внутри calculate_activity() возникнет исключение
            (БД, экспорт, Bridge или другая ошибка), старый спектр
            не должен повторно обрабатываться при следующем GetSpectre.

            Поэтому reset_spectrum() и spectrum_cycle_finished
            гарантированно выполняются через finally.
        """
        ...
        try:
            ...
            if mode == "RadDose":
                ...
                # ====================================================
                    ...
                    zb_object = self._build_zb_raddose_update(
                        card=card,
                        serial_number=sn,
                        temperature=temp_value,
                        paed=dose,
            ...
            # ========================================================
                ...
                # ----------------------------------------------------
                # Последнее корректное значение температуры
                # ----------------------------------------------------
                ...
                # ----------------------------------------------------
                # Для CZ фиксируем актуальность температуры
                # ----------------------------------------------------
            ...
            # ========================================================
                ...
                if spectrum_ready:
                    ...
                    # ====================================================
                    # КРИТИЧЕСКАЯ ЗАЩИТА ЗАВЕРШЁННОГО СПЕКТРАЛЬНОГО ЦИКЛА
                    # ====================================================
                    #
                    # После spectrum_ready == True этот накопленный спектр
                    # должен быть обработан НЕ БОЛЕЕ ОДНОГО РАЗА.
                    #
                    # calculate_activity() выполняет не только расчёт,
                    # но и:
                    #
                    #     - обновление GUI;
                    #     - запись результатов в БД;
                    #     - экспорт;
                    #     - формирование данных для Bridge;
                    #     - отправку данных в Bridge.
                    #
                    # Если после обновления algorithm_history или GUI
                    # внутри calculate_activity() возникнет исключение,
                    # его штатный reset_spectrum() в конце метода может
                    # не выполниться.
                    #
                    # Тогда старый spectrum_start_monotonic останется,
                    # условие времени при следующем GetSpectre снова
                    # будет выполнено, и один и тот же цикл может быть
                    # рассчитан повторно.
                    #
                    # Для алгоритма A это особенно опасно:
                    # algorithm_history уже может содержать результат
                    # предыдущего вызова, поэтому повторная обработка
                    # способна ошибочно сделать 99mTc доступным.
                    #
                    # finally гарантирует:
                    #
                    #     1. старый спектр будет сброшен;
                    #     2. локальный spectrum_active станет False;
                    #     3. DeviceManager получит сигнал завершения;
                    #     4. следующий расчёт возможен только после
                    #        нового StartSpectre и нового полного
                    #        времени накопления.
                    # ====================================================
                    ...
                    try:
                        ...
                        # calculate_activity() при нормальном завершении
                        # сам вызывает reset_spectrum().
                        #
                        # Повторный reset_spectrum() безопасен.
                        #
                        # При исключении этот вызов становится аварийной
                        # гарантией очистки незавершённого состояния.

                        card.reset_spectrum()

                        # Обязательно освобождаем спектральное состояние
                        # также на стороне DeviceManager.
                        #
                        # Иначе DeviceManager может продолжить считать,
                        # что прибор всё ещё находится в старом цикле.
    ...
    def sync_devices_with_cisterns(self):
        """
        Синхронизирует GUI-карточки ZB с текущим состоянием
        self.cistern_dict.

        При реальном изменении состояния цистерны:
        ...
        выполняется:
            ...
            1. Обновление состояния GUI.
            2. Сброс текущего спектрального накопления.
            3. Сброс истории алгоритма.
            4. Очистка старого спектрального результата в Bridge/PLC.
            5. Запись системного события:
                cistern_filled
                cistern_drained

        ВАЖНО:

        После изменения содержимого цистерны результаты предыдущего
        спектрального анализа больше не относятся к текущему содержимому.

        Поэтому в Bridge необходимо явно передать:

            ready_to_drain = 0

        и очистить ВСЕ предусмотренные для данной ZB изотопные слоты.

        Формат каждого слота соответствует контракту
        ModBusBridgeService:

            id              - номер физического слота PLC;
            name            - код изотопа;
            activity        - активность;
        ...
        for serial_number, card in self.cards_by_sn.items():
            ...
            # --------------------------------------------------------
            # CZ не имеют состояния fullness.
            # --------------------------------------------------------
            ...
            try:
                ...
                # Это очень важно.
                #
                # Нельзя очищать спектральные результаты при каждом
                # периодическом чтении fullness.
                #
                # Очистка выполняется только при РЕАЛЬНОМ переходе:
                #
                #     empty -> full
                #
                # или:
                ...
                # ====================================================
                # 8. СБРАСЫВАЕМ ТЕКУЩИЙ СПЕКТРАЛЬНЫЙ ЦИКЛ
                # ============================================================
                ...
                # ============================================================
                #
                # После смены содержимого цистерны история предыдущего
                # содержимого использоваться больше не должна.
                # ====================================================
                ...
                # ====================================================
                # 10. ОПРЕДЕЛЯЕМ КОЛИЧЕСТВО ИЗОТОПНЫХ СЛОТОВ PLC
                # ============================================================
                #
                # Согласно контракту ModBusBridgeService:
                #
                #     ZB1-ZB2 : 2 слота
                #     ZB3     : 5 слотов
                #     ZB4-ZB9 : 3 слота
                ...
                # ====================================================
                # 11. ФОРМИРУЕМ ПОЛНЫЙ МАССИВ ОЧИСТКИ ИЗОТОПОВ
                # ============================================================
                #
                # ВАЖНО:
                #
                # id НЕ является кодом изотопа.
                #
                # id — номер слота PLC.
                #
                # Для очистки слота:
                ...
                #     concentration = 0.0
                #
                # Пустой массив isotopes передавать нельзя:
                # старые значения PLC тогда могут остаться.
                # ====================================================

                isotopes_reset = []

                for isotope_id in range(
                    1,
                    isotope_count + 1
                ):

                    isotopes_reset.append(
                        {
                            "id": isotope_id,
                            "name": 0,
                            "activity": 0.0,
                ...
                # ====================================================
                # 12. ФОРМИРУЕМ ЧАСТИЧНОЕ ОБНОВЛЕНИЕ ZB
                # ============================================================
                #
                # SN, PAED, temperature и состояния детекторов здесь
                # не передаём.
                #
                # Их значения не изменились из-за смены fullness.
                #
                # Меняем только данные, которые относятся непосредственно
                # к предыдущему содержимому цистерны:
                #
                #     ready_to_drain
                #     isotopes
                # ====================================================

                spectrum_reset_update = {
                    "number": posit,
                    "ready_to_drain": 0,
                    "isotopes": isotopes_reset
                }

                # ====================================================
                # 13. ПЕРЕДАЁМ ОЧИСТКУ В BRIDGE
                # ============================================================
                #
                # _send_zb_data() одновременно обновит локальный кэш.
                #
                # Поэтому если Bridge сейчас отключён:
                #
                #     - состояние не потеряется;
                #     - в кэше уже будут нулевые результаты;
                #     - после reconnect _resync_bridge_state()
                #       повторно передаст их в Bridge.
                # ====================================================
                ...
                if new_full:
                    ...
                    self.ui.textEdit.append(
                        (
                            ...
                            "Спектр, історію та попередні "
                            "результати ідентифікації скинуто."
                        )
                    )
                ...
                # ====================================================
                # 15. ФОРМИРУЕМ СИСТЕМНОЕ СОБЫТИЕ
                # ============================================================

                if new_full:

                    event_type = "cistern_filled"

                    event_description = (
                        f"Цистерна ZB{posit} заповнена."
                    )

                else:

                    event_type = "cistern_drained"

                    event_description = (
                        f"Цистерна ZB{posit} спорожнена."
                    )

                # ====================================================
                # 16. СОХРАНЯЕМ СИСТЕМНОЕ СОБЫТИЕ
                # ============================================================
                #
                # Ошибка БД не должна отменять уже выполненную
                # синхронизацию состояния цистерны.
                # ====================================================
                ...
                try:
                    ...
                    self.ui.textEdit.append(
                        (
                            "[DB] Помилка збереження системної події "
                            f"'{event_type}' для ZB{posit}, "
                            f"SN {serial_number}: {event_error}"
                        )
                    )

            # ========================================================
            # 17. ОШИБКА ОДНОЙ ZB НЕ ОСТАНАВЛИВАЕТ ОСТАЛЬНЫЕ
            # ========================================================
    ...
    def cleanup(self):
        ...
        if (
            ...
            try:
                ...
                self.ui.textEdit.append(
                    "[Bridge] Закриття з'єднання з Bridge..."
                )

                self.modbus_client.shutdown()

                self.ui.textEdit.append(
                    "[Bridge] З'єднання з Bridge закрито"
                )
                ...
                self.ui.textEdit.append(
                    (
                        "[Bridge] Помилка завершення "
                        f"ModBusBridgeClient: {e}"
                    )
                )
    ...
    def on_device_connection_status(
        ...
        """
        Обрабатывает текущее состояние связи прибора.

        Правила:

            connected=False
                -> текущий запрос завершился timeout;
                -> GUI показывает отсутствие связи;
                -> для CZ последнее числовое значение температуры
                сохраняется, но temperature_valid снимается.

            connected=True, crc_error=True
                -> пакет физически получен, но повреждён;
                -> это НЕ считается восстановлением после
                подтверждённой потери прибора.

            connected=True, crc_error=False
                -> получен корректный пакет.

        Событие connection_restored создаётся только для прибора,
        который ранее был подтверждённо признан потерянным после
        5 последовательных неответов.

        В текущей штатной логике после 5 неответов такой прибор
        больше не опрашивается и далее должен проходить процедуру
        замены. Проверку connection_restored здесь сохраняем как
        защитный механизм на случай корректного ответа, пришедшего
        до окончательного прекращения обмена.
        """

        # ============================================================
        # 1. НАХОДИМ GUI-КАРТОЧКУ ПРИБОРА
        # ============================================================

        card = self.cards_by_sn.get(
            serial_number
        )

        # ============================================================
        # 2. ОБНОВЛЯЕМ GUI-СОСТОЯНИЕ СВЯЗИ
        # ============================================================

        if card:

            card.set_connection_status(
                connected,
                crc_error
            )

            # ========================================================
            # 3. TIMEOUT ДЛЯ НАСТЕННОГО ПРИБОРА CZ
            # ========================================================
            #
            # Последнее успешно полученное числовое значение
            # температуры НЕ уничтожаем.
            #
            # Timestamp последнего успешного измерения также
            # НЕ изменяем.
            #
            # Но после потери связи мы больше не можем считать
            # сохранённую температуру актуально подтверждённой.
            #
            # Поэтому следующая запись RadDose в БД получит:
            #
            #     temperature_valid = False
            #
            # при сохранённом:
        ...
        # ============================================================
        # 4. ПРОВЕРЯЕМ НАЛИЧИЕ СПИСКА ПОДТВЕРЖДЁННО
        #    ПОТЕРЯННЫХ ПРИБОРОВ
        # ============================================================

        if not hasattr(
            self,
            "_confirmed_offline_devices"
        ):
            return

        # ============================================================
        # 5. ПРИБОР НЕ БЫЛ ПОДТВЕРЖДЁННО ПОТЕРЯН
        # ============================================================

        if (
            serial_number
            not in self._confirmed_offline_devices
        ):
            return

        # ============================================================
        # 6. ВОССТАНОВЛЕНИЕМ СЧИТАЕТСЯ ТОЛЬКО
        #    ПОЛНОСТЬЮ КОРРЕКТНЫЙ ОТВЕТ
        # ============================================================
        ...
        # ============================================================
        # 7. УДАЛЯЕМ ПРИБОР ИЗ МНОЖЕСТВА OFFLINE
        # ============================================================

        self._confirmed_offline_devices.discard(
            serial_number
        )

        # ============================================================
        # 8. СОХРАНЯЕМ СИСТЕМНОЕ СОБЫТИЕ
        # ============================================================
        ...
        try:
            ...
            self.on_system_event(
                ...
                "connection_restored",
                (
                    "Зв'язок з приладом відновлено "
                    "після підтвердженої втрати зв'язку."
                )
            )

        except Exception as e:

            # Ошибка БД не должна ломать обработку GUI
            # или цикл работы программы.

            self.ui.textEdit.append(
                (
                    "[DB] Помилка збереження системної події "
                    f"'connection_restored' для SN "
                    f"{serial_number}: {e}"
                )
            )
    ...
    def on_device_missing(self, serial_number: str):
        """
        Обрабатывает подтверждённый выход прибора из строя.

        Метод вызывается DeviceManager только после пяти
        последовательных неответов прибора.

        После этого:

            1. прибор считается подтверждённо offline;
            2. незавершённый спектральный цикл удаляется;
            3. оператору разрешается выполнить замену прибора;
            4. запоминается SN неисправного прибора;
            5. в состояние Bridge записывается device_connection = 0.

        ВАЖНО:

            device_connection = 0 должен попасть в локальный Python-кэш
            даже тогда, когда Bridge в данный момент отключён.

            Поэтому здесь нельзя делать ранний:

                if not self.is_bridge_connected():
                    return

            и нельзя напрямую использовать:

                modbus_client.send_zb(...)
                modbus_client.send_cz(...)

            Для передачи используются _send_zb_data() / _send_cz_data(),
            которые сначала обновляют локальное состояние Python.
        """

        # ============================================================
        # 1. МНОЖЕСТВО ПОДТВЕРЖДЁННО НЕИСПРАВНЫХ ПРИБОРОВ
        # ============================================================
        ...
        if not hasattr(
            ...
            self._confirmed_offline_devices = set()

        self._confirmed_offline_devices.add(
            serial_number
        )

        # ============================================================
        # 2. РАЗРЕШАЕМ ОПЕРАТОРУ ЗАМЕНУ ПРИБОРА
        # ============================================================

        if (
            hasattr(self, "butt_replace_device")
            and self.butt_replace_device
        ):
            self.butt_replace_device.setEnabled(
                True
            )

        # ============================================================
        # 3. ЗАПОМИНАЕМ SN НЕИСПРАВНОГО ПРИБОРА
        # ============================================================

        self.missing_device_sn = serial_number

        # ============================================================
        # 4. ИЩЕМ GUI-КАРТОЧКУ ПРИБОРА
        # ============================================================
        ...
        if card is None:
            ...
            self.ui.textEdit.append(
                (
                    "Помилка обробки втрати зв'язку: "
                    f"картку приладу SN {serial_number} не знайдено."
                )
            )
        ...
        # ============================================================
        # 5. ПРЕРЫВАЕМ НЕЗАВЕРШЁННЫЙ СПЕКТРАЛЬНЫЙ ЦИКЛ
        # ============================================================
        #
        # Один обычный timeout НЕ сбрасывает спектр.
        #
        # Но сюда мы попадаем только после ПЯТИ последовательных
        # неответов, когда прибор уже окончательно признан неисправным.
        #
        # Поэтому незавершённый спектр такого прибора больше нельзя
        # использовать или продолжать после его замены.
        # ============================================================

        if (
            isinstance(card, DeviceCardBarrel)
            and getattr(
                card,
                "spectrum_active",
                False
            ...
            self.ui.textEdit.append(
                (
                    "Накопичення спектру перервано: "
                    f"прилад SN {serial_number} "
                    "не відповів 5 разів поспіль."
                )
            )

        # ============================================================
        # 6. ПРОВЕРЯЕМ НОМЕР ПОЗИЦИИ
        # ============================================================
        ...
        try:
            ...
            self.ui.textEdit.append(
                (
                    "Помилка передачі стану несправного приладу "
                    f"SN {serial_number}: "
                    f"некоректна позиція '{card.posit_number}'."
                )
            )
        ...
        # ============================================================
        # 7. ФОРМИРУЕМ ЧАСТИЧНОЕ ОБНОВЛЕНИЕ
        # ============================================================
        ...
        # ============================================================
        # 8. ПРИБОР ЦИСТЕРНЫ ZB
        # ============================================================

        if card.location_type == "cistern":
            ...
            if not 1 <= position <= 9:
                ...
                self.ui.textEdit.append(
                    (
                        "Помилка передачі стану несправного приладу "
                        f"SN {serial_number}: "
                        f"позиція ZB{position} поза діапазоном 1..9."
                    )
                )
            ...
            # --------------------------------------------------------
            # Используем именно _send_zb_data().
            #
            # Даже если Bridge сейчас отсутствует, этот метод сохранит
            # device_connection=0 в _bridge_zb_state.
            #
            # После восстановления Bridge п.26 выполнит resync и PLC
            # получит правильное состояние прибора.
            # --------------------------------------------------------

            if not self._send_zb_data(
                [connection_update]
            ):

                # Отсутствие Bridge здесь НЕ является потерей состояния:
                # оно уже сохранено в Python-кэше.
                self.ui.textEdit.append(
                    (
                        "[ZB] Стан втрати зв'язку збережено локально "
                        f"для SN {serial_number}, ZB{position}. "
                        "Передача в PLC буде виконана після "
                        "відновлення Bridge."
                    )
                )
        ...
        if card.location_type == "room":
            ...
            if not 1 <= position <= 3:
                ...
                self.ui.textEdit.append(
                    (
                        "Помилка передачі стану несправного приладу "
                        f"SN {serial_number}: "
                        f"позиція CZ{position} поза діапазоном 1..3."
                    )
                )
            ...
            # --------------------------------------------------------
            # Аналогично ZB состояние сначала попадает
            # в локальный _bridge_cz_state.
            # --------------------------------------------------------

            if not self._send_cz_data(
                [connection_update]
            ):

                self.ui.textEdit.append(
                    (
                        "[CZ] Стан втрати зв'язку збережено локально "
                        f"для SN {serial_number}, CZ{position}. "
                        "Передача в PLC буде виконана після "
                        "відновлення Bridge."
                    )
                )
        ...
        self.ui.textEdit.append(
            (
                "Помилка передачі стану несправного приладу "
                f"SN {serial_number}: "
                f"невідомий location_type '{card.location_type}'."
            )
        )
    ...
    def open_replace_dialog(self):
        """
        Открывает диалог замены приборов.

        Для процедуры ЗАМЕНЫ в список разрешённых приборов попадают
        только те активные приборы, которые ранее были окончательно
        признаны неисправными и находятся в
        self._confirmed_offline_devices.

        Это предотвращает случайную замену исправного прибора.

        Если одновременно отказало несколько приборов, оператор
        сможет выбрать любой из них.

        Неактивные приборы передаются отдельно для существующей
        функции их ручной активации.
        """

        # ============================================================
        # 1. ПОЛУЧАЕМ МНОЖЕСТВО ПОДТВЕРЖДЁННО НЕИСПРАВНЫХ ПРИБОРОВ
        # ============================================================
        ...
        # ============================================================
        # 2. ЗАМЕНА РАЗРЕШЕНА ТОЛЬКО ПОСЛЕ ПОДТВЕРЖДЁННОГО ОТКАЗА
        # ============================================================
        ...
        if not failed_serials:
            ...
            QMessageBox.warning(
                ...
                "Замена прибора",
                (
                    "Нет приборов, для которых подтверждён отказ связи.\n\n"
                    "Замена исправного прибора не разрешена."
                )
            )
        ...
        # ============================================================
        # 3. ПОЛУЧАЕМ АКТИВНЫЕ ПРИБОРЫ ИЗ БД
        # ============================================================

        active_devices = (
            self.db_manager.get_all_active_devices()
        )

        # ============================================================
        # 4. ОСТАВЛЯЕМ ТОЛЬКО ПОДТВЕРЖДЁННО НЕИСПРАВНЫЕ
        # ============================================================
        ...
        for device in active_devices:
            ...
            if serial_number in failed_serials:
                failed_devices.append(
                    device
                )

        # ============================================================
        # 5. ЗАЩИТА ОТ РАССИНХРОНИЗАЦИИ СОСТОЯНИЯ И БД
        # ============================================================
        ...
        if not failed_devices:
            ...
            QMessageBox.warning(
                ...
                "Замена прибора",
                (
                    "Подтверждённо неисправные приборы "
                    "не найдены среди активных приборов БД."
                )
            )
        ...
        # ============================================================
        # 6. ПОЛУЧАЕМ НЕАКТИВНЫЕ ПРИБОРЫ
        # ============================================================

        inactive_devices = (
            self.db_manager.get_inactive_devices()
        )
        ...
        # ============================================================
        #
        # В active_devices диалога передаём НЕ все активные приборы,
        # а только подтверждённо неисправные.
        #
        # Поэтому combo_old_device физически не сможет предложить
        # оператору исправный прибор для замены.
        # ============================================================
    ...
    def _identify_group_A(self, arr1, arr2, real_time, volume, history):
        ...
        if not history_valid:
            ...
            # identify_isotopes_alim() передає сюди T = 3600 с.
            ...
            history_updated = {
                ...
                # Прямий режим не використовується.
                # Поле залишено для сумісності структури history.
                "tc_use_direct": False,
        ...
        # ============================================================
        #
        # За контрольним Excel консультанта для кожного циклу >= 2
        # використовується динамічна формула:
        #
        #     S_Tc = (S_current - K_F * S_previous) / (K_Tc - K_F)
        #
        # Перемикання на прямий метод за ratio НЕ виконується.
        ...
        # ratio зберігаємо тільки для діагностики.
        if abs(sum_Tc_clean_current) > 1e-12:
            tc_ratio = sum_Tc_clean / sum_Tc_clean_current
        else:
            tc_ratio = None

        # Поле залишаємо для сумісності структури history/result.
        # Алгоритм завжди залишається динамічним.
        tc_use_direct = False
    ...
    def _identify_group_B(self, arr1, arr2, real_time, volume):
        ...
        ВАЖЛИВО:
            ...
            використовується сума каналів 90–150 з ARR_2,
            відповідно до контрольного Excel консультанта;
    ...
    def _identify_group_reserve(
        ...
        # використовується ARR_2[90..150] згідно з Excel.
        sum_I_raw = (
            self._calculate_window_sum(
                arr2,
                window_I[0],
    ...
    def identify_isotopes_alim(
        ...
        За контрольними Excel консультанта:
        ...
        # ============================================================
        #
        # За контрольними Excel консультанта для всіх трьох груп
        # математичний час алгоритму фіксований:
        #
        #     A       -> T = 3600 с
        #     B       -> T = 3600 с
        #     reserve -> T = 3600 с
        #
        # Фактичний час ПК зберігається окремо лише для діагностики.

        if group in ("A", "B", "reserve"):
            calculation_time = 3600.0
        else:
            self.ui.textEdit.append(
                f"Помилка ALIM: невідома група цистерни '{group}'."
            )
            return None, safe_history
    ...
    def _build_zb_raddose_update(
        ...
        """
        Формирует ОБЫЧНОЕ обновление ZB после пакета RadDose.

        Этот пакет обновляет только оперативные параметры детектора:
        ...
        Эти поля являются результатом спектрального анализа
        содержимого цистерны и не должны обнуляться каждым
        очередным измерением ПАЕД.

        Последний корректный спектральный результат должен
        сохраняться до:

            - следующего спектрального расчёта;
            - изменения состояния цистерны full/empty;
            - другого явного технологического сброса.
        """

        # ============================================================
        # 1. ПРОВЕРЯЕМ КАРТОЧКУ
        # ============================================================

        if card is None:
            raise ValueError(
                "Cannot build ZB RadDose update: card is None"
            )

        # ============================================================
        # 2. ПРОВЕРЯЕМ ПОЗИЦИЮ ZB
        # ============================================================
        ...
        try:
            ...
            raise ValueError(
                "Cannot build ZB RadDose update: invalid position"
            ) from exc

        if not 1 <= position <= 9:
            raise ValueError(
                (
                    "Cannot build ZB RadDose update: "
                    f"ZB position {position} is outside range 1..9"
                )
            )
        ...
        # ============================================================
        # 4. ФОРМИРУЕМ ЧАСТИЧНОЕ ОБНОВЛЕНИЕ
        # ============================================================
        #
        # ready_to_drain и isotopes здесь специально отсутствуют.
        # ============================================================
    ...
    def _send_zb_data(self, zb_list):
        """
        Сохраняет и отправляет обновления ZB в ModBusBridgeService.

        ВАЖНО:

        Последнее известное состояние каждой позиции ZB сохраняется
        локально независимо от того, подключён Bridge или нет.

        Благодаря этому после временной потери соединения можно
        повторно передать PLC актуальное состояние всех ZB.

        Поддерживаются как:

            - полные объекты ZB;
            - частичные обновления, например:
        ...
        Частичное обновление объединяется с ранее сохранённым
        состоянием этой позиции и не уничтожает остальные поля.
        """
        ...
        if not isinstance(zb_list, list):
            self.ui.textEdit.append(
                "[ZB] Помилка: очікується список даних ZB."
            )
            return
        ...
        # ============================================================
        # 2. СОЗДАЁМ КЭШ ПРИ ПЕРВОМ ИСПОЛЬЗОВАНИИ
        # ============================================================

        if not hasattr(self, "_bridge_zb_state"):
            self._bridge_zb_state = {}

        valid_updates = []

        # ============================================================
        # 3. ОБНОВЛЯЕМ ЛОКАЛЬНЫЙ СНИМОК ZB
        # ============================================================

        for update in zb_list:

            if not isinstance(update, dict):
                self.ui.textEdit.append(
                    "[ZB] Помилка: некоректний об'єкт ZB."
                )
                continue
            ...
            try:
                ...
                self.ui.textEdit.append(
                    "[ZB] Помилка: некоректний номер ZB."
                )
                continue

            if not 1 <= number <= 9:
                self.ui.textEdit.append(
                    f"[ZB] Помилка: номер ZB{number} поза діапазоном 1..9."
                )
                continue

            # --------------------------------------------------------
            # Получаем уже известное состояние позиции.
            # --------------------------------------------------------

            current_state = dict(
                self._bridge_zb_state.get(
                    number,
                    {"number": number}
                )
            )
            ...
            # --------------------------------------------------------
            # Объединяем частичное обновление с сохранённым состоянием.
            # --------------------------------------------------------

            current_state.update(update)

            current_state["number"] = number

            # --------------------------------------------------------
            # Сохраняем полный актуальный снимок этой ZB.
            # --------------------------------------------------------

            self._bridge_zb_state[number] = current_state

            # В текущую отправку передаём именно полученное обновление.
        ...
        # ============================================================
        # 4. ЕСЛИ BRIDGE НЕ ПОДКЛЮЧЁН
        # ============================================================
        #
        # Данные уже сохранены в кэше.
        #
        # После восстановления соединения они будут автоматически
        # повторно отправлены через _resync_bridge_state().
        # ============================================================

        if not self.is_bridge_connected():
            self.ui.textEdit.append(
                "[ZB] Bridge не підключений, дані збережено для ресинхронізації"
            )
            return

        # ============================================================
        # 5. ОТПРАВЛЯЕМ ТЕКУЩЕЕ ОБНОВЛЕНИЕ
        # ============================================================

        try:

            if not self.modbus_client.send_zb(
                valid_updates
            ):

                self.ui.textEdit.append(
                    "[ZB] Помилка при відправці даних"
                )

        except Exception as e:

            self.ui.textEdit.append(
                f"[ZB] Виняток при відправці: {e}"
            )
    ...
    def _send_cz_data(self, cz_list):
        """
        Сохраняет и отправляет обновления настенных детекторов CZ
        в ModBusBridgeService.

        Последнее известное состояние каждой позиции CZ сохраняется
        даже при отсутствии соединения с Bridge.

        После восстановления соединения сохранённые состояния
        автоматически повторно передаются через
        _resync_bridge_state().

        ВАЖНО:

        В системе существуют только:

            CZ1
            CZ2
            CZ3

        Поэтому номер CZ обязательно проверяется до помещения
        данных в кэш и до отправки в Bridge.

        Это предотвращает возможность записи Bridge в Modbus-адреса,
        находящиеся за пределами предусмотренной области CZ.
        """
        ...
        if not isinstance(cz_list, list):

            self.ui.textEdit.append(
                "[CZ] Помилка: очікується список даних CZ."
            )
        ...
        if not hasattr(
            ...
            self._bridge_cz_state = {}

        valid_updates = []

        # ============================================================
        # 3. ПРОВЕРЯЕМ И СОХРАНЯЕМ КАЖДОЕ ОБНОВЛЕНИЕ
        # ============================================================

        for update in cz_list:

            # --------------------------------------------------------
            # Каждый элемент CZ должен быть JSON-совместимым словарём.
            # --------------------------------------------------------
            ...
            if not isinstance(
                ...
                self.ui.textEdit.append(
                    "[CZ] Помилка: некоректний об'єкт CZ."
                )

                continue

            # --------------------------------------------------------
            # Номер CZ является обязательным.
            # --------------------------------------------------------
            ...
            try:
                ...
                self.ui.textEdit.append(
                    "[CZ] Помилка: некоректний номер CZ."
                )

                continue

            # ========================================================
            # 4. СТРОГАЯ ПРОВЕРКА CZ1-CZ3
            # ========================================================
            #
            # В Bridge адрес вычисляется относительно номера CZ.
            #
            # Поэтому нельзя пропускать CZ0, CZ4 и любые другие
            # значения за пределами физически предусмотренных
            # CZ1-CZ3.
            # ========================================================
            ...
            if not 1 <= number <= 3:
                ...
                self.ui.textEdit.append(
                    (
                        "[CZ] Помилка: "
                        f"номер CZ{number} "
                        "поза допустимим діапазоном 1..3."
                    )
                )
            ...
            # ========================================================
            # 5. ПОЛУЧАЕМ ПРЕДЫДУЩЕЕ СОСТОЯНИЕ CZ
            # ========================================================

            current_state = dict(
                self._bridge_cz_state.get(
                    number,
                    {
                        "number": number
                    }
            ...
            # ========================================================
            # 6. ОБЪЕДИНЯЕМ ЧАСТИЧНОЕ ОБНОВЛЕНИЕ
            # ========================================================
            #
            # Например:
            #
            #     {"number": 1, "device_connection": 0}
            #
            # изменит только состояние связи и не уничтожит
            # сохранённые SN, PAED и температуру.
            # ========================================================
            ...
            # Номер позиции всегда берём уже после нашей проверки.
            current_state["number"] = number

            # ========================================================
            # 7. СОХРАНЯЕМ АКТУАЛЬНЫЙ СНИМОК CZ
            # ========================================================
            ...
            # В обычной текущей отправке передаём только реально
            # полученное обновление.
            valid_update = dict(
                update
            )

            valid_update["number"] = number
        ...
        # ============================================================
        # 8. ПОСЛЕ ПРОВЕРКИ НЕ ОСТАЛОСЬ ДАННЫХ
        # ============================================================
        ...
        #     _resync_bridge_state()
        #
        # повторно отправит последнее состояние CZ.
        # ============================================================
        ...
        if not self.is_bridge_connected():
            ...
            self.ui.textEdit.append(
                (
                    "[CZ] Bridge не підключений, "
                    "дані збережено для ресинхронізації"
                )
            )
        ...
        # ============================================================
        # 10. ОТПРАВЛЯЕМ В BRIDGE
        # ============================================================

        try:

            if not self.modbus_client.send_cz(
                valid_updates
            ):

                self.ui.textEdit.append(
                    "[CZ] Помилка при відправці даних"
                )
    ...
    def _resync_bridge_state(self):
        """
        Повторно отправляет в ModBusBridgeService последнее известное
        состояние всех ZB и CZ.

        Метод вызывается после восстановления соединения:

            Python -> Bridge

        Цель:

            PLC не должен оставаться со старыми значениями только потому,
            что часть измерений произошла в момент отсутствия Bridge.

        ВАЖНО:

        Отправляются именно сохранённые последние состояния каждой
        позиции, а не искусственно сформированные нулевые значения.
        """

        # ============================================================
        # 1. УБЕЖДАЕМСЯ, ЧТО BRIDGE ДЕЙСТВИТЕЛЬНО ПОДКЛЮЧЁН
        # ============================================================
        ...
        for number in sorted(zb_state.keys()):

            state = zb_state.get(number)
        ...
        for number in sorted(cz_state.keys()):

            state = cz_state.get(number)
        ...
        if zb_snapshot:
            ...
            try:
                ...
                if self.modbus_client.send_zb(
                    zb_snapshot
                ):

                    self.ui.textEdit.append(
                        (
                            "[Bridge] Ресинхронізація ZB завершена: "
                            f"{len(zb_snapshot)} позицій."
                        )
                    )
                    ...
                    self.ui.textEdit.append(
                        "[Bridge] Помилка ресинхронізації ZB."
                    )
                ...
                self.ui.textEdit.append(
                    (
                        "[Bridge] Виняток при ресинхронізації ZB: "
                        f"{e}"
                    )
                )
        ...
        if cz_snapshot:
            ...
            try:
                ...
                if self.modbus_client.send_cz(
                    cz_snapshot
                ):

                    self.ui.textEdit.append(
                        (
                            "[Bridge] Ресинхронізація CZ завершена: "
                            f"{len(cz_snapshot)} позицій."
                        )
                    )
                    ...
                    self.ui.textEdit.append(
                        "[Bridge] Помилка ресинхронізації CZ."
                    )
                ...
                self.ui.textEdit.append(
                    (
                        "[Bridge] Виняток при ресинхронізації CZ: "
                        f"{e}"
                    )
                )
