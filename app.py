...
class App(QObject):
    ...
    def on_bridge_finished(
        ...
        Если процесс Bridge завершился, активного PLC-сеанса
        больше не существует.

        Поэтому:

            - прекращаем reconnect Python-клиента;
            - сбрасываем состояние подключения;
            - оба индикатора делаем красными;
            - кнопку возвращаем в Connect PLC.
        """

        # ============================================================
        # 1. ФОРМИРУЕМ ТЕКСТ ПРИЧИНЫ ЗАВЕРШЕНИЯ
        # ============================================================
        ...
        # ============================================================
        # 2. ПРОЦЕССА BRIDGE БОЛЬШЕ НЕТ
        # ============================================================

        self.bridge_process = None

        # ============================================================
        # 3. АКТИВНЫЙ СЕАНС СЧИТАЕМ ЗАВЕРШЁННЫМ
        # ============================================================

        self._plc_connection_requested = False

        # ============================================================
        # 4. ОСТАНАВЛИВАЕМ АВТОМАТИЧЕСКИЙ RECONNECT
        # ============================================================
        #
        # Без работающего ModBusBridgeService reconnect к локальному
        # TCP-порту бессмысленен и может продолжаться бесконечно.
        # ============================================================
            ...
            try:
                ...
                self.ui.textEdit.append(
                    (
                        ...
                        f"локального з'єднання: {e}"
                    )
                )

        # ============================================================
        # 5. СБРАСЫВАЕМ ИНДИКАТОРЫ
        # ============================================================
    ...
    def on_bridge_error(self, error):
        """
        Обрабатывает ошибку процесса ModBusBridgeService.

        После ошибки процесса текущий сеанс подключения
        ...
        # ============================================================
        # 2. АКТИВНЫЙ СЕАНС ЗАВЕРШЁН
        # ============================================================

        self._plc_connection_requested = False

        # ============================================================
        # 3. ОСТАНАВЛИВАЕМ RECONNECT К НЕРАБОТАЮЩЕМУ BRIDGE
        # ============================================================
        ...
        # ============================================================
        # 6. QPROCESS БОЛЬШЕ НЕ СЧИТАЕМ РАБОЧИМ
        # ============================================================
    ...
    def on_connect_plc_clicked(self):
        ...
        Кнопка работает как переключатель:
            ...
            Connect PLC
                -> запускаем ModBusBridgeService;
                -> начинаем соединение Python -> Bridge;
                -> разрешаем автоматический reconnect.

            Disconnect PLC
                -> запрещаем автоматический reconnect;
                -> отключаем Python от Bridge;
                -> останавливаем ModBusBridgeService;
                -> сбрасываем оба индикатора.

        ВАЖНО:

        Состояние кнопки означает, что пользователь запросил
        активный сеанс связи с PLC через Bridge.

        Поэтому при временном обрыве Python -> Bridge кнопка
        остаётся "Disconnect PLC", пока Bridge работает и
        ModBusBridgeClient выполняет автоматическое восстановление.
        """

        # ============================================================
        # 1. ПРОВЕРЯЕМ, АКТИВЕН ЛИ УЖЕ СЕАНС СВЯЗИ
        # ============================================================
        ...
        # ============================================================
        # 2. ЕСЛИ СЕАНС УЖЕ АКТИВЕН — ЭТО КОМАНДА DISCONNECT
        # ============================================================
        ...
        if (
            ...
            # --------------------------------------------------------
            # Сначала запрещаем дальнейшее автоматическое
            # восстановление данного сеанса.
            # --------------------------------------------------------
            ...
            #
            # disconnect() также запрещает автоматический reconnect
            # до следующего connect_to_bridge().
            # --------------------------------------------------------
            ...
            # --------------------------------------------------------
            # Завершаем сам процесс ModBusBridgeService.
            # --------------------------------------------------------
            ...
            # --------------------------------------------------------
            # После ручного Disconnect связь считается отсутствующей
        ...
        # ============================================================
        # 5. ФИКСИРУЕМ, ЧТО ПОЛЬЗОВАТЕЛЬ ЗАПРОСИЛ СОЕДИНЕНИЕ
        # ============================================================
        #
        # Этот флаг используется для различия:
        #
        #   - временного обрыва с автоматическим reconnect;
        #   - ручного Disconnect;
        #   - аварийного завершения Bridge.
        # ============================================================
        ...
        if not self.start_bridge(
            ...
            # Процесс даже не удалось запустить.
        ...
        # ============================================================
        # 8. РЕЗУЛЬТАТ ПЕРВИЧНОГО ПОДКЛЮЧЕНИЯ
        # ============================================================
        ...
        if connected:
            ...
            # Сигнал connected дополнительно вызовет
            # on_bridge_connected().
            ...
            # ВАЖНО:
            #
            # Здесь НЕ возвращаем кнопку в Connect PLC.
            #
            # Сам Bridge уже запущен, а ModBusBridgeClient имеет
            # внутренний reconnect-worker и продолжит попытки
            # подключения автоматически.
            #
            # Пользователь при желании может остановить этот процесс
            # кнопкой Disconnect PLC.
            # --------------------------------------------------------
    ...
    def on_bridge_connected(self):
        """
        Обрабатывает успешное соединение:

            Python -> ModBusBridgeService

        После установки соединения:

            1. Bridge считается подключённым;
            2. PLC пока не считается подтверждённо подключённым;
            3. кнопка показывает Disconnect PLC;
            4. выполняется полная ресинхронизация последнего
            известного состояния ZB/CZ.
        """

        # ============================================================
        # 1. АКТИВНЫЙ СЕАНС СВЯЗИ
        # ============================================================
        ...
        # ============================================================
        #
        # Подтверждён только Python -> Bridge.
        #
        # Фактическое состояние Bridge -> PLC пока отдельно
        # не подтверждено.
        # ============================================================
        ...
        # ============================================================
        # 5. ВОССТАНАВЛИВАЕМ СОСТОЯНИЕ ZB/CZ
        # ============================================================
        #
        # Если во время отсутствия Bridge приходили новые измерения
        # или изменился device_connection, эти значения уже находятся
        # в локальном кэше.
        # ============================================================
    ...
    def on_bridge_data_received(self, data):
        ...
        Bridge может прислать НЕПОЛНЫЙ список ZB, если чтение одного
        или нескольких состояний PLC завершилось ошибкой.

        В этом случае:

            1. корректно прочитанные ZB продолжаем использовать;
            2. отсутствующие ZB НЕ считаем пустыми;
            3. старое состояние отсутствующих ZB НЕ изменяем;
            4. оператору явно сообщаем, какие ZB в текущем
            цикле PLC не были подтверждены.

        Ошибка одной записи не должна останавливать обработку остальных.
        """
        ...
        try:
            ...
            if not zb_list:
                ...
                self.ui.textEdit.append(
                    "[Bridge] Увага: отримано порожній "
                    "список станів ZB."
                )
            ...
            # ============================================================
            # 5. ПРОВЕРЯЕМ ВСЕ ПОЛУЧЕННЫЕ СОСТОЯНИЯ
            # ============================================================
            ...
            for zb in zb_list:
                ...
                # --------------------------------------------------------
                # number должен быть именно int.
                #
                # bool специально не принимаем, хотя bool является
                ...
                # --------------------------------------------------------
                # fullness должен быть строго 0 или 1
                # --------------------------------------------------------
                ...
                # --------------------------------------------------------
                # Дубли одной ZB в одном сообщении недопустимы
                # --------------------------------------------------------
            ...
            # ============================================================
            # 7. ПРОВЕРЯЕМ ПОЛНОТУ СНИМКА PLC
            # ============================================================
            #
            # Bridge должен попытаться прочитать ZB1...ZB9.
            #
            # Если какой-либо номер отсутствует среди успешно
            # проверенных данных, его состояние в этом цикле
            # НЕ подтверждено PLC.
            #
            # ВАЖНО:
            #
            # Мы НЕ устанавливаем такое состояние в False.
            # Последнее известное значение сохраняется до отдельной
            # обработки потери/восстановления PLC в п.25.
            # ============================================================
            ...
            if missing_numbers:
                ...
                self.ui.textEdit.append(
                    (
                        "[Bridge] Увага: у поточному циклі "
                        "PLC не підтвердив стан: "
                        f"{missing_text}. "
                        "Збережено попередній стан."
                    )
                )

            # ============================================================
            # 8. ЕСЛИ НЕТ НИ ОДНОГО ДОСТОВЕРНОГО СОСТОЯНИЯ
            # ============================================================
            ...
            # ============================================================
            # 9. ОБНОВЛЯЕМ ТОЛЬКО ИЗМЕНИВШИЕСЯ СОСТОЯНИЯ
            # ============================================================
            ...
            # ============================================================
            # 10. ЕСЛИ ФИЗИЧЕСКОЕ СОСТОЯНИЕ НЕ ИЗМЕНИЛОСЬ
            # ============================================================
            #
            # Проверка полноты ответа уже выполнена выше.
            # Поэтому теперь можно закончить обработку.
            # ============================================================
            ...
            # ============================================================
            # 11. АТОМАРНО СОХРАНЯЕМ cistern.json
            # ============================================================
            ...
            try:
                ...
                # --------------------------------------------------------
                # PLC уже сообщил актуальное состояние.
                #
                # Поэтому оперативное состояние в памяти не откатываем.
                # --------------------------------------------------------
            ...
            # ============================================================
            # 12. СИНХРОНИЗИРУЕМ GUI
            # ============================================================

            self.sync_devices_with_cisterns()

            # ============================================================
            # 13. ПЕРЕДАЁМ СНИМОК В DeviceManager
            # ============================================================
    ...
    def on_device_connection_status(
        ...
        Дополнительно фиксирует системное событие
        "connection_restored", но только если ранее прибор был
        подтверждённо признан потерянным после 5 последовательных
        неответов.

        Это принципиально важно:

            один или несколько обычных timeout
            НЕ считаются полной потерей связи;

            CRC-ошибка НЕ считается восстановлением связи;

            событие восстановления создаётся только один раз
            при первом корректном ответе после подтверждённого offline.
        """

        # ============================================================
        # 1. ОБНОВЛЯЕМ GUI-КАРТОЧКУ ПРИБОРА
        # ============================================================
        ...
        # ============================================================
        # 2. ЕСЛИ НЕТ СПИСКА ПОДТВЕРЖДЁННО ПОТЕРЯННЫХ ПРИБОРОВ
        # ============================================================
        #
        # Значит после запуска программы ещё ни один прибор
        # не был признан offline по правилу 5 неответов подряд.
        ...
        # ============================================================
        # 3. ПРОВЕРЯЕМ, БЫЛ ЛИ ЭТОТ ПРИБОР ПОДТВЕРЖДЁННО OFFLINE
        # ============================================================
        ...
        # ============================================================
        # 4. ПРОВЕРЯЕМ, ЧТО ПОЛУЧЕН ИМЕННО КОРРЕКТНЫЙ ОТВЕТ
        # ============================================================
        #
        # connected=True, crc_error=True означает, что данные
        # физически пришли от линии, но пакет имеет ошибку CRC.
        #
        # Такое состояние ещё нельзя считать восстановлением
        # нормальной связи.
        #
        # Восстановление признаём только при:
        #
        #     connected == True
        #     crc_error == False
        ...
        # ============================================================
        #
        # Сначала удаляем прибор из множества offline.
        #
        # Благодаря этому следующие успешные ответы не будут
        # создавать повторные события connection_restored.
        ...
        try:
            ...
            # Ошибка записи системного события не должна ломать
            # обработку восстановившегося прибора и GUI.
    ...
    def on_device_missing(self, serial_number: str):
        """
        Обрабатывает подтверждённую потерю связи с прибором.

        Этот метод вызывается DeviceManager только после того,
        как прибор не ответил установленное количество раз подряд.

        Выполняются четыре задачи:

            1. прибор запоминается как подтверждённо offline;
            2. активируется возможность замены прибора;
            3. serial_number сохраняется для диалога замены;
            4. в Bridge немедленно передаётся:

                device_connection = 0

            чтобы PLC/HMI не продолжал считать потерянный
            прибор подключённым.

        ВАЖНО:

        connection_error здесь повторно не создаётся.
        Оно уже формируется DeviceManager в момент подтверждения
        потери связи.
        """

        # ============================================================
        # 1. ЗАПОМИНАЕМ ПРИБОР КАК ПОДТВЕРЖДЁННО OFFLINE
        # ============================================================
        ...
        # ============================================================
        # 3. СОХРАНЯЕМ SN ДЛЯ ДИАЛОГА ЗАМЕНЫ
        # ============================================================

        self.missing_device_sn = serial_number

        # ============================================================
        # 4. НАХОДИМ GUI-КАРТОЧКУ ПРИБОРА
        # ============================================================
        #
        # Из карточки нам нужны:
        #
        #     location_type
        #     posit_number
        #
        # чтобы определить, относится прибор к ZB или CZ.
        ...
        if card is None:
            ...
            self.ui.textEdit.append(
                (
                    "[Bridge] Неможливо передати втрату зв'язку "
                    f"для SN {serial_number}: картку приладу не знайдено."
                )
            )
        ...
        # ============================================================
        # 5. ЕСЛИ BRIDGE НЕ ПОДКЛЮЧЁН
        # ============================================================
        #
        # Локальная программа всё равно корректно считает прибор
        # offline.
        #
        # Передать это состояние в PLC сейчас невозможно.
        # Механизм полной ресинхронизации после reconnect будет
        # отдельно проверяться далее в цепочках Bridge/PLC.

        if not self.is_bridge_connected():
            return

        # ============================================================
        # 6. ФОРМИРУЕМ МИНИМАЛЬНОЕ ОБНОВЛЕНИЕ
        # ============================================================
        #
        # Bridge уже поддерживает частичные обновления объектов ZB:
        # например ready_to_drain передаётся отдельно только вместе
        # с number.
        #
        # Поэтому здесь не пересылаем старые значения PAED,
        # температуры, активности и т.д.
        #
        # Передаём только то состояние, которое действительно
        # изменилось:
        #
        #     device_connection = 0
        ...
        if card.location_type == "cistern":
            ...
            try:
                ...
                if not self.modbus_client.send_zb(
                    ...
                    self.ui.textEdit.append(
                        (
                            "[ZB] Помилка передачі втрати зв'язку "
                            f"для SN {serial_number}, "
                            f"ZB{card.posit_number}."
                        )
                    )
                ...
                self.ui.textEdit.append(
                    (
                        "[ZB] Виняток при передачі втрати зв'язку "
                        f"для SN {serial_number}: {e}"
                    )
                )
        ...
        if card.location_type == "room":
            ...
            try:
                ...
                if not self.modbus_client.send_cz(
                    ...
                    self.ui.textEdit.append(
                        (
                            "[CZ] Помилка передачі втрати зв'язку "
                            f"для SN {serial_number}, "
                            f"CZ{card.posit_number}."
                        )
                    )
                ...
                self.ui.textEdit.append(
                    (
                        "[CZ] Виняток при передачі втрати зв'язку "
                        f"для SN {serial_number}: {e}"
                    )
                )
